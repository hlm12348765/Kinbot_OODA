...
建议下一轮优先围绕 `KBT-57` 和仍未关闭的高代价开放问题继续清账：

1. `Q-013`：后台服务 / 人工坐席是否与机器人本体产品联动立项，服务模式与 `SLA` 如何定义？
2. `Q-014`：非隐私结构化数据回流如何分级、授权、脱敏、限用和留存？
3. `Q-015`：是否打开 `10000` 元 BOM 与 `29999` 元定价分支，新增成本买来什么？
4. `Q-016`：首批 `10000` 台、销售 + 租赁并行和渠道让利压缩如何落到交付与资金模型？
5. `Q-006`：如何冻结 `C5` 工作基线，并显式记账 `C4 -> C1 / C2 / C5` 的成本与功耗转移？
