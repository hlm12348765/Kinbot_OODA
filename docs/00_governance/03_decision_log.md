...
文档变更记录：
- v3.14 | 2026-07-26 | Codex-架构师 | 同步 L1 评审后的协作状态：`KBT-59` 完成并关闭，`KBT-58` 与 `KBT-60` 解除前置阻断，分别承接找物功能映射和 P2 `S1-S7` 重映射。
- v3.13 | 2026-07-26 | Codex-架构师 | 补记 A-123：用户通过 L1 最终评审并冻结当前“基础业务 Agent + TaskWorker + RobotSkill”基线；Q-018 关闭，L2 承接内部状态机、协议、数据、阈值、算法和部署。同步修正涌现编号，并把找物已进入移动、跨房间紧急输入退化和旧移动安全撤销纳入端到端压力场景，不改变 L1 实体与责任。
- v3.12 | 2026-07-26 | Codex-架构师 | 补记 A-122：以聊天、日程、找物和突发健康事件组成端到端压力场景，验证紧急抢占、到人递药、外部联系、持续观察和责任交接；场景未发现新的 L1 实体或旁路，同时明确 A0 直接创建根级有界 TaskWorker、无机械臂和外部急救／医疗服务可用性边界。
- v3.11 | 2026-07-26 | Codex-架构师 | 补记 A-121：明确 RobotSkillSystem 执行语义感知的 `P-P1/I-P1`，O3 从 `I2/Evidence` 开始执行 `I-P2`；增加 L0 过程／操作数到 L1 的完整映射和第七类“持续感知与共享状态更新闭环”。
- v3.10 | 2026-07-25 | Codex-架构师 | 补记 A-120：当前 L1 架构进入最终评审；固定重大事件先回到 TaskWorker、再按根任务影响升级 A0 的正常因果链，明确 AP3 的异步确定冷启动原则及后续案例校正要求，展开四个基础业务 Agent 的受控直接协调，并以四个反例完成文档级收口。L2 承接状态机、协议、字段、阈值和算法。
- v3.9 | 2026-07-25 | Codex-架构师 | 补记 A-119：确认基于 A-117／A-118 的“基础业务 Agent + TaskWorker + RobotSkill”路线推进；当前文档不再使用“方案三”名称，历史候选移入历史记录，精确拓扑继续由 Q-018 验证。
- v3.8 | 2026-07-25 | Codex-架构师 | 补记 A-118：限制 G3 的替换权，统一候选—实际操作—执行结果反馈；以完成判据和下一步前提判断是否主动推送 Owner，并把方案三关系显式归纳为六类闭环。
- v3.7 | 2026-07-25 | Codex-架构师 | 以 A-117 替代原 A-116 方案三：所有 Agent 统一复用 Physical Agent 七元组；四个基础业务对应长期 AgentCell，具体任务由临时 TaskWorker 承担，基础能力通过 RobotSkill 复用；加入 O3 共享上下文、主动授权、受控直接协调、AP3 运行机制和单点失效验证。
- v3.6 | 2026-07-23 | Codex-架构师 | 补记方案三执行链决策：A0 不逐动作审批，N3/OP3 的候选经 AP3 租约校验，G3 按许可事件裁决并返回结构化理由，F1 在独立实时周期输出明确安全动作；各层采用独立时钟，OP3 以同一 PhysicalAgentCell 的可选未激活状态预留。
- v3.5 | 2026-07-23 | Codex-架构师 | 将方案三收敛为 L1 优先候选：Agent Cell 以“任务契约的唯一 Owner”为决定性门槛；A0/C3/N3/OP3 分别承担根任务、照护、导航和未来操作契约，O3/AP3 共享 StateRef/EvidenceRef，N3/OP3 直接读取局部实时 I1，并以 Owner 租约、隔离令牌和故障续行明确分层最终决定权；方案一、方案二转为 superseded 对照。
- v3.4 | 2026-07-23 | Codex-架构师 | 补齐 L1 方案三的系统范围：按响应时间与硬约束、跨领域上下文、状态持续时间与恢复责任三个轴，明确 A0、O3、C3、N3、OP3、ST3、G3、AP3、F1 的候选责任和 A0 失效时的受限紧急目标合同；三案进入同范围比较。
- v3.3 | 2026-07-22 | Codex-架构师 | 细化 L0 已确认关系：`I-P1` 是把 `I1` 解释为 `I2` 的唯一顶层过程；`P-P2～P-P7` 可直接读取经过登记的实时 `I1`，用于确定性控制、限制、中止和保护，不因此新增顶层过程或绕过目标与许可边界。
- v3.2 | 2026-07-22 | Codex-架构师 | 将物理智能体统一形式记录为 \((\mathcal S,\mathcal X,\mathcal Y,\mathcal A,\phi,M,\Psi)\) 七元组；三套 L1 方案保持相同外部闭环，只比较状态估计、跨领域决策、领域闭环、语义授权和硬安全在内部如何分配。
- v3.1 | 2026-07-22 | Codex-架构师 | 记录 Agent 母定义拉齐和 L1 方案三：广义 Agent 以感知—行动闭环定义，独立责任只用于判断是否单列 Agent Cell；`KBT-59` 新增混合分层自治方案，与方案一、方案二并列评审。
- v3.0 | 2026-07-21 | Codex-架构师 | 为 `KBT-59` 增加 L1 方案二：单一常驻 A0、O1/ST1/AP1/G1/F1 五个非 Agent 系统和按需临时子 Agent；与既有 `A0-A4 + AP1 + F1` 方案一并列比较，等待用户决策。
- v2.9 | 2026-07-20 | Codex-架构师 | 更新 KBT-59 的 L1 候选：由八个并列软件 Agent 收敛为 A0 主 Agent、A1-A3 专业 Agent、A4 独立安全与授权 Agent，以及 AP1/F1 两个非 Agent 平台；同步主业务规划、状态语义和安全边界。
- v2.8 | 2026-07-20 | Codex-架构师 | 记录 L0 架构收口：用户通过纸面枚举、连接和领域聚类全部信息 / 物理过程与操作数，独立回归到 `OODA + Feedback + Management`，确认 16 号文档第 3、4 章；L1 精确拓扑仍由 `KBT-59` 继续评审。
- v2.7 | 2026-07-17 | Codex-架构师 | 根据读者测试拆分递归 Agentic 迁移的状态：A-112 只确认系统与软件责任域方向；A-113 / Q-018 / `KBT-59` 承接七构件 / 五项判据、`F1 + A1-A8`、四个收敛机制、确定性叶子分类与三层在线护栏，并阻塞下游映射冻结。
- v2.6 | 2026-07-17 | Codex-架构师 | 记录递归式 Agentic Robot System 迁移决策：Kinbot 作为跨信息 / 物理空间的超级 Agent，每个软件运行时子系统统一采用 Agentic 设计；`F1 + A1-A8`、确定性叶子边界和在线层级护栏进入新 L1 文档评审。
- v2.5 | 2026-06-27 | Codex-架构师 | 补记 `V1` 机载药箱 P2 决策稿工作口径：推荐前向轻量电动浅抽屉 / 托盘 + 最小取放检测进入工程细化，并明确防夹、锁止、开仓不移动、取出 / 服用拆分和 Phase 5 证据字段边界。
- v2.4 | 2026-04-21 | Codex-架构师 | 新增系统组成权衡模型判断：以 `12` 类组成对象、`7` 个价值维度、`9` 个资源维度和双成本情景统一评审成本、重量、尺寸、药箱、屏幕、交互、运动性能、服务 / 坐席与数据治理取舍。
- v2.3 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：记录后台服务 / 人工坐席、非隐私数据回流、成本定价上修、首批 `10000` 台与租售并行的战略输入，并建立 `KBT-57` 作为活跃 `provisional` 承接项。
- v2.2 | 2026-04-14 | Codex-架构师 | 补记 `Step 50` 的头部方案决策：`V1` 默认冻结为“紧凑轻量头部 + 躯干内容屏”，并明确高集成度带屏头只作为头颈高速链路阶段门不过线时的 `Plan B`。
...
| F-148 | `V1` 头部默认布局方向   | `V1` 默认冻结为“紧凑轻量头部 + 躯干内容屏”：头部优先承载 `1` 组双目、必要单目、单主麦阵与小尺寸表情屏，`8 到 10` 寸内容显示屏下沉到躯干，高集成度带屏头不再作为默认主线                                                                                   | confirmed   | `input/00_requirements/00_user_requirements_input.md` Step 50、用户本轮确认 |
| F-149 | 头部方案 `Plan B` 触发条件 | 只有当方案二在头颈高速链路、`EMI`、反复摆动寿命与装配 / 可维护性阶段门不过线，且必须引入过贵或过脆弱的线束 / 机构方案时，才回退到方案一的高集成度带屏头                                                                                           | confirmed   | `input/00_requirements/00_user_requirements_input.md` Step 50、用户本轮确认 |
| F-150 | 董事长对后台服务 / 坐席的战略反馈 | 汇报反馈显示董事长对云服务、后台服务和人工坐席的重视程度高于当前估计，认为机器人从一开始就应考虑后台服务 / 人工坐席，甚至需要与机器人本体产品一起立项                                                                                  | confirmed   | 用户本轮转述董事长汇报反馈 |
| F-151 | 非隐私数据回流战略反馈    | 董事长反馈认为非隐私性数据可以回流，用于机器人和后台服务的持续进化；但原始视觉、语音、生物特征等敏感数据仍受既有端侧处理与受控回流边界约束                                                                                                  | confirmed   | 用户本轮转述董事长汇报反馈，既有数据边界 |
| F-152 | 成本、定价与首批商业模式战略反馈 | 董事长反馈提出第一代希望功能做得更好，不强求 `6000` 元成本，可评估 `10000` 元 BOM；定价可从 `23999` 上修到 `29999`；减少渠道让利，首批可按 `10000` 台，并同时探索销售与租赁模式                              | confirmed   | 用户本轮转述董事长汇报反馈 |
| F-153 | `KBT-57` 承接边界      | 已建立 `KBT-57`《拆解董事长反馈：后台服务/人工坐席立项联动与成本定价商业口径》，用于承接服务 / 坐席联动立项、非隐私数据回流、`10000` 元 BOM、`29999` 元定价、首批 `10000` 台和租售并行的活跃 `provisional` | confirmed   | Linear `KBT-57` |
| F-154 | L1 评审协作状态         | `KBT-59` 已完成递归式 Agentic Robot System 的 L1 评审并关闭；`KBT-58` 与 `KBT-60` 不再受其前置阻断，分别承接找物功能映射和 P2 `S1-S7` 重映射                                      | confirmed   | 用户 2026-07-26 明确通过 L1 评审并要求维护相关 Issue |
...
| A-099 | 在当前开发承接层，`V1` 已确认主闭环仍收缩为“机器人本体 + 家属 App + 最小云”；人工服务、在线问诊、第三方履约与公共应急尚未进入已确认主链验收，其中人工服务 / 坐席已由 `A-106 / KBT-57` 重新评估是否前置联动立项 | confirmed | `codex/v1-real-complexity-reduction` 本轮文档收口，`A-106` |
...
| A-087 | 历史口径：为避免架构层级继续膨胀，一代曾把长期记忆、技能化、连接器与受控任务编排压缩为跨模块 Agent 增强平面；该组织方式已由 A-112 的递归式 Agentic 架构覆盖 | superseded | `input/00_requirements/00_user_requirements_input.md` Step 41, A-112 |
...
| A-104 | 在头部三自由度被定义为一代差异化资产的前提下，`V1` 应优先保住头部轻量化、角速度上限与姿态表达上限，而不是优先追求头部大模组化和躯干简化 | confirmed | `input/00_requirements/00_user_requirements_input.md` Step 50、用户本轮确认 |
| A-105 | 方案二的关键风险属于头颈高速链路工程阶段门，而不是先验否决理由；若视频链路稳定性、`EMI`、寿命与可维护性不过线，再回退高集成度带屏头更符合当前主线的复杂度控制纪律 | confirmed | `input/00_requirements/00_user_requirements_input.md` Step 50、用户本轮确认 |
| A-106 | 后台服务 / 人工坐席不能继续只按低优先级“后续适配位”处理；当前应升级为 `KBT-57` 承接的立项联动候选，但在组织、`OPEX`、`SLA`、服务边界和数据治理拆解完成前，不改写已确认 `V1` 主闭环 | provisional | F-150, `KBT-57` |
| A-107 | 数据回流必须拆成“敏感原始数据默认不回流”和“非隐私结构化数据可受控回流”两类，不得用持续进化需求反向放松原始视觉、语音和生物特征的端侧处理原则 | confirmed | F-151, 既有数据边界 |
| A-108 | 当前 `5000 到 6000` 元整机 `BOM` 冻结基线暂不被覆盖；`10000` 元 BOM 和 `29999` 元定价只作为 `KBT-57` 战略分支，必须先证明新增成本能转化为用户可感知功能、服务闭环、可靠性或商业模型收益 | provisional | F-152, `KBT-57` |
| A-109 | 董事长提出的首批 `10000` 台、销售 + 租赁并行属于商业放大假设，不替代当前 `100 台 / 100 户 / 1 个月` MVP 试点基线；必须先通过试点、供应、交付、售后、坐席和资金占用测算 | provisional | F-152, `KBT-57`, `docs/05_p4_beta_dvt/01_mvp_validation_plan.md` |
| A-110 | 系统组成权衡应采用“硬门槛 -> 价值评分 -> 资源消耗 -> 风险转移 -> 双成本情景”的统一模型，而不是让成本、重量、尺寸、药箱、屏幕、交互、运动性能等各自局部最优；当前统一落到 `12` 类组成对象、`7` 个价值维度、`9` 个资源维度和 `5000 到 6000 / 10000` 两个成本情景 | confirmed | `docs/03_p2_feasibility/08_system_tradeoff_model_and_priority_matrix.md` |
| A-111 | `V1` 机载药箱当前按 P2 工作口径推荐采用“前向轻量电动浅抽屉 / 托盘 + 最小取放检测”：无机械臂、用户自取 / 放回，机器人负责到人、驻停、开仓、提示、检测、记录和升级；`已取出` 与 `已服用` 必须拆分，防夹、锁止、开仓不移动和物品取放状态是工程硬门槛，多格自动盘点、机械臂递药、云侧视觉裁决和药箱 `ToF` 不进入默认主线 | confirmed | `docs/03_p2_feasibility/10_v1_onboard_medicine_box_decision_draft.md` |
| A-112 | Kinbot 确认向递归式 Agentic Robot System 迁移：机器人是跨信息空间与物理空间的超级 Agent；广义 Agent 以“通过传感器感知环境、通过执行器作用于环境”的闭环定义，不要求大模型、非确定性、长期记忆或常驻进程。物理智能体统一写成 \((\mathcal S,\mathcal X,\mathcal Y,\mathcal A,\phi,M,\Psi)\) 七元组；L1/L2 Agent Cell 的决定性门槛是成为一项持续任务契约的唯一 Owner，并对目标、状态、验证、恢复和结果负责。旧“九模块 + Agent 增强平面”不再定义未来软件组织方向 | confirmed | 用户 2026-07-22、2026-07-23 当前对话明确指令，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` |
| A-113 | 方案一 `A0-A4 + AP1 + F1` 作为历史对照保留，不再作为当前 L1 主线候选。它形式上有 A0，但 A1-A3 已先形成领域解释与判断，A0 再依据跨 Agent 摘要裁决，容易损失高维模态和不确定性；若领域决定可直接生效，又会与 A0 的最终决定权冲突。使用共享证据引用、唯一契约 Owner 并按业务责任重切边界后会向 A-117 演变 | superseded | 用户 2026-07-23 与 2026-07-25 当前对话判断，A-117，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture_history.md`“历史方案一” |
| A-114 | L0 的系统边界、形式与功能，以及 `I1～I8`、`I-P1～I-P8`、`P1～P6`、`P-P1～P-P7` 的过程—操作数划分和关系确认为当前基线；用户通过纸面枚举、连接和按实际工作领域聚类，独立回归到 `OODA + Feedback + Management`。其中，`I-P1` 是把 `I1` 解释为 `I2` 的唯一顶层过程；`P-P2～P-P7` 可以直接读取经过登记的实时 `I1`，用于确定性控制、限制、中止和保护，但不得借此生成语义状态、扩大目标或越过许可边界。该确认不包含过程到 L1 实体的精确责任分配 | confirmed | 用户 2026-07-20、2026-07-22 当前对话明确确认，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 3、4 章 |
| A-115 | 方案二“一个常驻 A0 + O1/ST1/AP1/G1/F1”作为历史对照保留，不再作为当前 L1 主线候选。若 ST1 严格只是技能系统，A0 失效后已接受任务没有可继续履约的局部 Owner；若给具体任务持续状态、验证和恢复责任，它们应展开为 A-117 的 TaskWorker，而不是把所有真实复杂度继续藏在 ST1 中 | superseded | 用户 2026-07-23 与 2026-07-25 当前对话判断，A-117，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture_history.md`“历史方案二” |
| A-116 | 原方案三“契约式混合自治”：按 A0、C3、N3、OP3 划分根任务、照护、导航和未来操作契约 Owner。该方案确认了唯一 Owner、共享证据、多速率闭环和 G3／F1 分离，但把技术能力域与业务契约域混在同一 L1 分解维度，已由 A-117 的“基础业务 Agent + TaskWorker + RobotSkill”替代 | superseded | 用户 2026-07-25 当前对话批准重构，A-117，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 5～7 章 |
| A-117 | 当前 L1 采用“基础业务 Agent + TaskWorker + RobotSkill”：所有 Agent 严格复用 Physical Agent 七元组；A0 持有根任务和用户承诺，健康与照护、陪伴与关系、家庭安全、环境与物品四个基础业务 Agent 持有长期业务责任，具体任务由临时 TaskWorker 负责，交互、导航、识别、记忆访问、信息服务和未来操作统一作为 RobotSkill 复用。O3 管理 SharedState、SharedMemory 和 Evidence，AP3 管 TaskWorker 运行机制，G3 产生语义许可与理由，F1 直接读取实时物理信号并执行硬安全。主动任务必须经 A0 签发 BusinessAuthorization；基础业务 Agent 只可在有效根契约、授权和不扩权前提下通过 CoordinationGraph 直接协调。TaskWorker 和 Skill 内部 Agent 均是同型 AgentCell，但只拥有有界任务或执行子契约。A0、O3、AP3、G3、RobotSkill 运行时、共享模型、CoordinationGraph、F1 和共同部署故障必须通过可证伪的单点失效验证 | confirmed | 用户 2026-07-25 批准实施，用户 2026-07-26 通过 L1 评审，A-123，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 5～11 章 |
| A-118 | A-117 的外部操作闭环按以下规则补齐：ActionProposal 同时携带状态不确定性、允许修正范围、预先允许的等价操作、完成判据、下一步前提和不可改变约束；G3 只能批准、在允许范围内约束、选择预先允许的等价操作、拒绝或撤销，不能创造新的业务行动。PermitDecision 必须返回实际操作、候选—实际差异、理由和影响程度；直接提案者必收，影响完成判据、下一步前提、外部影响、权限、预算、时限或任务终态时主动推送契约 Owner，只有影响根任务、跨业务优先级或用户承诺时升级到 A0。TaskWorker 只依据实际操作、SkillOutcome、ExecutionDeviation、SharedState 和 Evidence 更新状态。当前 L1 关系归纳为 Agent Cell 局部、任务与契约履约、信息操作、物理任务、F1 硬安全、运行与治理、持续感知与共享状态更新七类闭环 | confirmed | 用户 2026-07-25 批准实施，用户 2026-07-26 通过 L1 评审，A-117，A-121，A-123，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 5.3、7.6～7.8 节，`docs/02_p1_architecture/07_safety_compliance_authorization_api.md` |
| A-119 | 确认基于 A-117／A-118 定义的“基础业务 Agent + TaskWorker + RobotSkill”路线继续推进。该路线从此作为当前 L1 架构直接命名，不再称为“方案三”；原方案一、方案二及其公式、图示和否决依据迁入 16 号历史记录，不再与当前架构并列。此决策确认路线选择，不等于冻结精确拓扑；场景、故障、时延、资源和组织验证仍由 Q-018／KBT-59 承接 | confirmed | 用户 2026-07-25 当前对话明确确认，A-117，A-118，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md`，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture_history.md` |
| A-120 | 当前 L1 架构形成最终评审基线：正常重大事件必须先反馈当前 TaskWorker，由 TaskWorker 判断能否在原契约内恢复；只有影响根任务、跨业务优先级或用户承诺时，才停止新的外部影响并升级 A0，正常情况下不存在 RobotSkillSystem、G3 或 F1 直接向 A0 发起业务升级的路径。AP3 使用异步、确定且不解释业务语义的冷启动原则，后续依据业务场景和冲突案例持续校正。四个基础业务 Agent 可通过受控协调关系直接交接有界子契约，AP3 管机制、O3 提供共享引用，无法在根契约内解决的冲突才返回 A0。导航微调、任务级安全中断、首次资源冲突和首次跨业务协作四个反例未发现新的 L1 实体或未归属决定权；内部状态机、协议、字段、阈值和算法转入 L2。该决策确认最终评审内容，不等于冻结 L1 | confirmed | 用户 2026-07-25 当前对话明确确认，A-117～A-119，`KBT-59`，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 7.4～7.7、11 章 |
| A-121 | L0 的持续感知过程在当前 L1 中按以下责任闭合：F1 承担机电与硬安全信号的 `P-P1`，RobotSkillSystem 的感知运行时承担视觉、声音等语义感知的 `P-P1`，感知、交互和连接器 RobotSkill 执行 `I-P1` 并产生 `I2/Evidence`；O3 从 `I2/Evidence` 开始执行 `I-P2`，维护 `I3`，不得把 `P1/I1` 直接提升为共享事实。显式意图 `I2` 同时供 O3 更新上下文、供 A0 建立根契约。AP3 保障持续感知的实例和资源，G3 约束采集范围、用途和隐私，但二者不解释感知语义。L1 新增“持续感知与共享状态更新闭环”，使全部 `I-P/P-P` 过程与 `I/P` 操作数能逐项映射到 L1 实体、闭环和时间尺度；该责任边界已纳入 A-123 的 L1 冻结基线 | confirmed | 用户 2026-07-26 明确同意执行并通过 L1 评审，A-114，A-117～A-120，A-123，`docs/02_p1_architecture/16_recursive_agentic_robot_system_architecture.md` 第 5.4、6.3、7.2、7.7 节 |
| A-122 | 聊天、创建日程、寻找钥匙和帽子、突发胸部绞痛与呼救、紧急返回、到人确认、递药、联系家属／急救服务、持续观察和责任交接可以由当前 L1 七类实体及动态 TaskWorker 组织，不需要新增基础业务 Agent 或正常旁路。没有稳定业务归属且不满足基础业务判据的根级有界任务可由 A0 直接创建 TaskWorker，A0 只承担父契约责任，不能积累该领域的长期状态或执行局部规划。找物调用 Navigation Skill 后已经经 G3、F1 进入实际移动，机器人可能与用户分处不同房间；紧急输入必须区分近场观察、低置信远场声音和 App／穿戴求救事件，并在抢占时撤销旧找物移动许可、由 F1 安全停车。该场景同时确认两项产品能力前提：V1 无机械臂，只能递送已由人放入机载仓且用户可自行取用的药品；公共急救、医院、在线问诊和人工坐席只有在连接器与服务可用性冻结后才能成为可承诺闭环 | confirmed | 用户 2026-07-26 要求复杂场景验证、补充跨房间触发并通过 L1 评审，A-111，A-117～A-121，A-123，`docs/02_p1_architecture/16_end_to_end_emergency_scenario_appendix.md` |
| A-123 | 当前“基础业务 Agent + TaskWorker + RobotSkill”L1 架构通过最终评审并冻结。冻结范围包括 A0、四个基础业务 Agent、O3、RobotSkillSystem、AP3、G3、F1、动态 TaskWorker 的实体、责任、主关系、七类闭环和系统不变量；L2 继续展开内部状态机、协议、数据、阈值、算法、部署和验证。9.1 节涌现编号由未定义且与历史 Module 编号冲突的 `M1～M7` 改为 Emergence 对应的 `E1～E7`；端到端场景补齐找物移动、跨房间输入退化和旧移动安全撤销，只修正表达与场景完整性，不改变 L1 架构 | confirmed | 用户 2026-07-26 明确判断 L1 评审可以通过并要求完成两项小调整，A-117～A-122，Q-018 |
...
| Q-013 | 后台服务 / 人工坐席是否与机器人本体产品联动立项，采用自建、外包、合作还是混合模式，服务时段、响应 `SLA`、质检和 `OPEX` 如何定义 | 决定 `A-106` 能否从 `provisional` 升级为当前 `V1` 交付基线 | open |
| Q-014 | 非隐私结构化数据回流的分级、授权、脱敏、用途限制、留存周期和持续进化闭环如何定义 | 决定 `A-107` 如何落入 `S6 / S7`、隐私合规与模型 / 服务改进机制 | open |
| Q-015 | 是否打开 `10000` 元 BOM 与 `29999` 元定价分支，新增成本分别买来哪些功能、服务、可靠性、产品感或商业收益 | 决定 `A-108` 是否改写当前成本与选型主线 | open |
| Q-016 | 首批 `10000` 台、销售 + 租赁并行和渠道让利压缩如何拆解到试点、首销、租赁库存、交付、售后和资金占用模型 | 决定 `A-109` 是否能进入 `G5` 发布与商业准备门控 | open |
| Q-017 | 机载药箱前向浅抽屉 / 托盘方案能否通过防夹、锁止、开仓不移动、取放检测、寿命、噪声、装配和产测硬门槛 | 决定 `A-111` 是否能从 P2 工作口径进入后续结构冻结和 Phase 5 量产预备证据包 | open |
| Q-018 | A-120／A-121／A-122 的 L1 最终评审基线是否可以冻结：A0、四个基础业务 Agent、O3、RobotSkillSystem、AP3、G3、F1 与动态 TaskWorker 的实体、责任、关系和不变量是否完整；重大事件逐级升级、基础业务受控直接协调、七类闭环、L0—L1 过程／操作数映射、AP3 冷启动、四个反例和端到端压力场景的收口判断是否成立 | 已由 A-123 确认并冻结；状态机、协议、字段、阈值、算法、业务案例校正和外部服务能力冻结转由 L2 或产品／服务方案承接 | closed |

## 5. 下一轮优先向用户确认的问题

L1 已由 A-123 冻结。下一轮应确定 L2 的分解顺序与首个评审对象，再围绕 `KBT-57` 和仍未关闭的高代价开放问题继续清账：

1. L2：先展开哪一个 L1 实体或跨实体闭环，采用什么评审顺序？
2. `Q-013`：后台服务 / 人工坐席是否与机器人本体产品联动立项，服务模式与 `SLA` 如何定义？
3. `Q-014`：非隐私结构化数据回流如何分级、授权、脱敏、限用和留存？
4. `Q-015`：是否打开 `10000` 元 BOM 与 `29999` 元定价分支，新增成本买来什么？
5. `Q-016`：首批 `10000` 台、销售 + 租赁并行和渠道让利压缩如何落到交付与资金模型？
6. `Q-006`：如何冻结 `C5` 工作基线，并显式记账 `C4 -> C1 / C2 / C5` 的成本与功耗转移？
7. `Q-011`：`G5` 量产预备门的 `7` 域与 `7` 条通过条件是否需要进一步细化为阶段门量化项？
