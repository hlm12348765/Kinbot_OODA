...
文档变更记录：
- v2.2 | 2026-04-08 | Codex-架构师 | 收紧附录 B，不再展开第 8 条原则候选正文，只保留短指针，避免原则层继续膨胀为候选池。
...
1. `具身智能体的主动生成能力` 仍可作为后续战略讨论输入，但不进入已冻结的 `7` 条正式原则。
2. 如需继续讨论，统一在活跃评审总包与后续 issue 中承接，不在原则层继续扩写候选池。
