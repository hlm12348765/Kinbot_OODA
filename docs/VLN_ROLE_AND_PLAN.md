...
1. 是否正式确认 `VLN = 语义导航策略层`，而不是底层运动控制器。
   - **结论：是**
   - **理由：** VLN 在 OODA 框架中对应 Orient→Decide 层，负责语义判断并输出全局规划指令，局部避障及停止由 Navigation 处理。这与分析中的解耦架构一致。

2. 是否新增 `semantic_navigation_policy` 和 `semantic_spatial_memory` 两个逻辑能力。
   - **结论：semantic_navigation_policy 需要；semantic_spatial_memory 待补充详细解释**
   - **理由：** VLN 当前实现”去哪、找什么、找谁、搜索顺序”等语义导航策略能力。semantic_spatial_memory文章中并未详细解释，具体细节需进一步明确。

3. 是否把”指定人搜索 / 跟随 / 靠近”升级为与寻物同级的一条正式产品能力线。
   - **结论：是**
   - **理由：** 跟随与靠近人（类似 TrackVLA++），与寻物逻辑差异较大，不适于作为寻物功能的子集或合并开发。

4. 是否明确”借鉴 VLA 的训练与模型方法，但不照搬智驾式 `video -> action` 闭环”。
   - **结论：是**
   - **理由：** 自动驾驶端到端倾向明显强于机器人，机器人VLN 仍普遍采用分层架构。应先增强 VLM 空间理解能力，再做 VLN 集成，而非直接 E2E。

5. 是否新增 `social_mobility_policy`，专门覆盖无指令共存移动与主动 reposition。
   - **结论：长期来看是需要的**
   - **理由：** 无明确指令时机器人如何根据环境和用户行为做出合理运动决策（如走廊相遇礼让）属于”方向 B”（具身移动智能）。建议先用强化学习实现初步能力，分阶段推进。当前阶段以产品闭环为重点，此能力属于长期规划。

6. 是否采用”仿真层高保真 URDF + 策略层 compact embodiment_profile + 教师大模型 / 执行小模型双轨”的工程路线。
   - **结论：可做参考**
   - **理由：** 仿真层需 URDF 支持本体感知，是否高保真仍需更多测试数据做参考；
