...
按各工况中值估算，当前 `P_mix` 应控制在约 `34 到 42 W` 的收敛带内；若显著超出，需要优先从调度和交互策略回收，而不是先加大电池。
...
`Step24` 进一步明确：以后每次显著降本动作都要附带“高端产品感知检查表”，而且监控目标不再只写“约 30000 元”，而是按 `20000 到 30000 元` 售价区间持续监控。
...
| `温暖感` | 语音、灯光、表达、提醒是否有陪伴感 | 冷冰冰的提示器 |
| `精致感` | 外观、结构、屏幕、装配、细节是否高级 | 廉价 IoT 感 |
| `从容感` | 移动、转向、停靠、打断和恢复是否稳 | 急促、吵闹、慌乱 |
| `可信感` | 状态切换、响应、权限、安全是否可预期 | 行为突兀、不透明 |
| `服务感` | 本体与 App、云、坐席是否协同顺滑 | 关键价值都被外包到手机上 |
...
1. 以 `KBT-30` 为当前主线，先冻结四类工况下的 `C5` 工作基线。
2. 在 `docs/COST_STRUCTURE_AND_TECH_DOWNPATH.md` 中继续维护 `BOM + 售价` 双基线。
