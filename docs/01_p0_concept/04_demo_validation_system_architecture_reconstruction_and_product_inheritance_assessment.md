...
文档变更记录：
- v1.3 | 2026-03-22 | 用户 + Codex-架构师 | 吸收用户手动调整的本体实体架构视图，修正长焦相机的模块归属，并细化 `RK3576 / S100Pro / AT32F457VET7` 与各本体模块之间的连接关系。
- v1.2 | 2026-03-22 | Codex-架构师 | 按用户审阅意见修正 Demo 的“体系理念 / 技术理念”评价口径，并重画本体实体架构图，将算力与控制域拆成 `RK3576 / S100Pro / AT32F457VET7` 三个子模块及其归属连接。
- v1.1 | 2026-03-22 | Codex-架构师 | 吸收 Step38，确认三芯片职责分层、8 个独立视觉模组、双麦阵成因、真实实装电池与散热条件，并将整机重量修正为 50kg。
...
- 技术理念：不算很激进，更接近“用较传统、较稳妥的重传感和重冗余方案，去支撑激进的体系目标”。
- 工程实现：更像研发验证平台，不像量产基线。

这里需要特别区分两层含义：

1. 如果讨论的是“体系理念”，它是激进的，因为它试图在一台验证平台上同时跑通全向观察、安全冗余、递送、姿态表达和多芯片协同。
2. 如果讨论的是“技术方案本身”，它并不算今天意义上的激进路线，因为它主要依赖大量传感器和冗余器件来做全向视觉、全向主动传感，而不是依赖“少相机 + 多自由度观察 + 更强算法闭环”这类更前沿、更克制的方案。

因此，更准确的表述应该是：

`体系理念激进，但技术方案本身偏重冗余验证，不是最前沿的激进方案。`
...
结合 `Step38`，当前已经明确以下实现归属：

1. `RK3576` 用于交互主控，承担语音交互链路、屏幕显示、人脸识别、交互相机接入和 App 运行。
2. `S100Pro` 是算力主控，用于整机视觉接入、导航控制和算法运行。
3. `AT32F457VET7` 是 `MCU`，用于整机电机和传动控制（不含底盘），并接入 `IMU`、超声波等传感器。
4. `4` 组双目环视相机与 `4` 组超广角相机确实是 `8` 个独立视觉模组。

### 3.3 已确认的关键实现细节

1. `4` 组双目环视相机安装高度约 `75cm`，视野开阔，原本作为 `VLN` 模型的主观测输入。
2. `4` 组超广角相机安装高度约 `18cm`，原计划专用于地面观测，可交给 `VLN` 推理，也可交给 `SLAM` 匹配；但实际这 `4` 个相机的功能并没有真正用上。
3. 长焦相机同样在实际系统里没有真正用上。
4. `6` 麦阵位于头顶，但由于圆形大屏上沿高于头顶，形成遮挡和反射；因此又增加了颈部 `8` 麦阵来消除这一问题。
5. `466.2Wh + 被动散热` 是真实实装配置。
6. 整机重量前一版写成 `70kg` 是输入错误，当前正确值为 `50kg`。
...
5. 在高位双目视觉、低位地面视觉、主动传感、长焦和多执行器都齐备的前提下，快速判断哪些传感器和哪些自由度真正有系统价值。
...
这套 Demo 的感知设计不是平铺的，而是带有明确层次：

- 环视双目：负责空间与障碍主感知，也是 `VLN` 主观测输入
- 超广角：原计划负责地面和近距补盲
- 长焦：原计划负责中远距找人和识别
...
- 这套任务分层思路本身是对的；
- 但低位 `4` 超广角和长焦在实际系统中并没有真正形成有效价值。

所以后续真正应继承的是“按任务切感知”的方法，而不是默认继承“每一层都单独配一组器件”。
...
### 6.3 问题三：主动传感冗余太强，且实际有效价值没有和器件数量成正比
...
而这套 Demo 本质上是把主动传感器作为主感知骨架的一部分。结合 `Step38`，还可以进一步收敛出一个更关键的结论：

- 高位双目主观测是有系统价值的；
- 低位 `4` 超广角和长焦并没有真正被系统吃透。

这意味着产品路线更不能简单复制器件，而应复制“哪一类观测真的进入了决策主链”。
...
### 6.5 问题五：被动散热真实存在，但更说明它是一台验证平台而不是量产平台

`Step38` 已确认 `466.2Wh + 被动散热` 是真实实装配置。这个信息没有减轻问题，反而进一步说明：

- 这台 Demo 的工程平衡是围绕“先跑起来、先验证能力”组织的；
- 它并不是围绕“量产可持续热平衡、静音、重量和成本”组织的。

因此，这条真实配置应被理解为“验证平台特征”，而不是产品架构可直接继承的优点。

### 6.6 问题六：头部声学与大屏结构发生了明显冲突

`Step38` 证明了一个非常重要的架构事实：

- 头顶 `6` 麦阵因为圆形大屏上沿高于头顶，出现了遮挡和反射；
- 为了修补这个问题，系统又加了颈部 `8` 麦阵。

这说明该 Demo 已经暴露出一个明确的产品架构反模式：

`头部视觉 / 屏幕 / 声学 / 结构一体化没有在设计初期一起收敛，导致后续通过叠加器件修补。`

这个教训应该直接进入 Kinbot 主线。

### 6.7 问题七：速度指标偏验证思路，不偏养老产品思路
...
### 7.3 适宜沿用三：高位主观测优先、按任务而不是按器件去组织感知
...
后续可以用更少的相机来实现这组任务分层，但不应丢掉“高位主观测优先”这条思路。
...
### 8.6 必须抛弃六：用叠加器件修补头部结构与声学冲突

`Step38` 已经证明，头顶麦阵 + 大屏结构的冲突是这台 Demo 的真实问题。
...
- 头部视觉、屏幕、声学、表情和结构在一开始就联合收敛。

### 8.7 必须抛弃七：把 Demo 器件堆叠等同于系统架构
...
| 双目深度环视 * 4 | 不继承器件数量；转译成量产主线的双目 + 单目 `RGB` 方案，并优先保留高位主观测视角 |
| 固态激光雷达 * 4 | 仅保留为研发对照与真值链，不进入产品主线 |
| 长焦相机 * 1 | 当前不直接继承为默认量产件；因 Demo 中未真正用上，其价值应先回到“中远距识别能力是否刚需”重新论证 |
| 超广角相机 * 4 | 不直接继承数量；Demo 已表明这一路径并未真正跑通，后续只保留“地面补盲需求”而不保留原器件方案 |
