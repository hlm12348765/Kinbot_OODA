...
文档变更记录：
- v3.0 | 2026-07-17 | Codex-架构师 | 增加递归 Agentic 迁移声明：本文保留产品 / 部署 / 硬约束参考角色，不再作为软件拓扑唯一总图；当前 L1 候选总图由 `16` 承接评审。
- v2.9 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：后台服务 / 人工坐席进入 `KBT-57` 立项联动候选，非隐私结构化数据回流和成本定价上修仅作为待业务拆解战略假设，不覆盖当前 `V1` 主闭环。
...
> **迁移声明（2026-07-17）**：本文是上一轮总体架构与双视角参考。产品边界、部署边界和硬约束继续有效；旧九模块、Agent 增强平面及“唯一软件总图”表述已被递归 Agentic 方向覆盖。当前 L1 首要评审入口为 [16_recursive_agentic_robot_system_architecture.md](16_recursive_agentic_robot_system_architecture.md)，其精确拓扑仍为 `provisional / in review`。

---

## 1. 文档定位

本文档是上一轮总体架构参考，不再是当前软件拓扑的唯一总图；当前 L1 候选总图见 `16`。
...
6. 董事长汇报反馈要求重新评估后台服务 / 人工坐席是否需要与机器人本体产品联动立项；该项当前由 `KBT-57` 承接，尚未改写 `V1` 主闭环冻结口径
...
4. 后台人工服务 / 坐席从纯“后续适配位”升级为 `KBT-57` 承接的立项联动候选；在组织形态、`OPEX`、服务可用性和数据治理拆解完成前，不进入当前 V1 主闭环验收
...
上一轮总图在运行时视角只保留当时定义的稳定能力域和系统级硬边界。
...
3. 本文件按历史版本保留 `9` 个一级模块与它们之间的主干关系
...
5. `companion_service_system` 在当前 V1 中默认收缩为 `App + 最小云`；后台人工服务 / 坐席进入 `KBT-57` 立项联动候选，第三方接入继续只保留接口位
...
4. 后台人工服务 / 坐席由 `KBT-57` 承接是否联动立项；智能家居和第三方服务接入仍只保留后续适配位，不进入当前主闭环
5. 断网时运动安全与本地闭环不能失效
6. 原始敏感数据默认不回流；非隐私性结构化数据可在 `KBT-57` 中评估“授权 + 脱敏 + 加密 + 时效受限 + 用途受限”的受控回流与持续进化闭环
...
2. 系统边界约束：一代固定为轮式移动交互机器人，不包含机械臂和复杂物理操作；交付对象仍是“机器人 + 伴生系统 + 服务”的完整产品系统，但当前 V1 主闭环中的“服务”只指最小云侧同步、远程确认与审计；后台服务 / 人工坐席是否与本体产品联动立项由 `KBT-57` 承接，不等于已经默认启用人工服务或第三方履约
3. 决策与治理约束：优先级固定为“安全 > 合规 > 用户指令 > 任务完成率 > 效率 > 能耗”；动作执行前必须经过安全、合规、授权三道门
4. 数据与部署约束：原始视觉与语音数据默认端侧处理；运动安全与本地执行保护必须端侧闭环；断网不得影响运动与避障安全能力
5. 工程与商业约束：续航目标大于 `4` 小时，尺寸建议不超过 `50 x 50 x 120 cm`；当前已冻结整机物料成本基线仍为 `5000 到 6000 元`，董事长反馈中的 `10000` 元 BOM、`29999` 元定价、首批 `10000` 台和租售并行仅作为 `KBT-57` 待拆解战略分支；所有分支都不能破坏“聪明、温暖、精致”的高端产品感
...
8. `KBT-57` 是否能把后台服务 / 坐席、非隐私数据回流、成本上修、定价上修、渠道压缩、首批 `10000` 台和租售并行拆解成可执行商业与系统方案
...
4. `9` 个一级模块只作为历史责任映射输入；新的软件拓扑须在 `KBT-59` 中评审，部署边界和硬约束继续有效
5. 当前开发承接默认只围绕离散决策与事件驱动两条主链组织；连续流式与长周期演化只保留研究线和接口位
6. 本文件不再并列展开纯视觉硬件细节、三级能力模式、`Agent` 增强和四条业务闭环；这些内容继续保留，但降为附录说明或专题文档输入
7. 董事长反馈已改变服务链和商业口径的优先级，但当前只形成 `KBT-57` 战略假设承接；在用户确认业务执行拆解前，不把它写成已冻结新基线
