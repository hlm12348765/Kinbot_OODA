...
1. 机器人本体是目标系统核心
2. 穿戴、智能家居、家属应用、云服务、后台人工服务属于伴生系统
3. 外部医疗、药店、配送、社区和公共应急接口属于外部生态，不进入一代自建边界
4. 默认优先本地闭环，只有本地闭环不够时才进入服务升级链路
...
3. 断网时运动安全与本地闭环不能失效
4. 原始敏感数据默认不回流，但架构层继续允许预留“授权 + 脱敏 + 加密 + 时效受限 + 用途受限”的受控回流接口
...
2. 系统边界约束：一代固定为轮式移动交互机器人，不包含机械臂和复杂物理操作；交付对象必须是“机器人 + 伴生系统 + 服务”的完整产品系统
...
6. 资源与节奏约束：默认量产资源线仍按 `8GB RAM + 64GB Flash` 组织，`12GB + 64GB` 作为边界验证线，`16GB + 64GB` 及以上只作为前瞻验证线或未来 `Pro SKU` 候选；当前阶段目标仍是形成总体方案与模块下发基线
...
1. 产品系统边界继续保留为“机器人本体 + 伴生系统 + 外部生态接口”的完整产品系统
...
5. 本文件不再并列展开纯视觉硬件细节、三级能力模式、`Agent` 增强和四条业务闭环；这些内容继续保留，但降为附录说明或专题文档输入
...
3. `L3`：依赖 App、云、人工服务和第三方生态的服务扩展能力
