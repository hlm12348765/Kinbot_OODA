...
5. `EVT` 前允许引入深度相机和激光雷达作为验证加速线，但 `EVT` 后默认退出一代量产主线。
...
4. **数据与部署约束**  
   原始视觉与语音数据必须端侧处理；运动安全与本地执行保护必须在端侧闭环；断网不得影响运动与避障安全能力。

5. **工程与商业约束**  
   续航目标大于 4 小时，尺寸建议不超过 `50 x 50 x 120 cm`，整机物料成本 `6000 到 8000 元`；同时整机与服务组合不能破坏“聪明、温暖、精致”的高端产品感。
