...
文档变更记录：
- v1.1 | 2026-03-26 | Codex-架构师 | 将业务主状态中的“保姆协同”下沉为横切角色/任务模式，正式提升“家庭安全巡护”为业务主状态，并补入任务上下文栈与状态切换快照等状态机协同构件。
...
- 首发场景是中国大陆居家养老
- 首版主价值排序为“健康管理 > 陪伴交互 > 家庭安全巡护 > 老人看护”
...
- `R1` 到 `R4` 四类 OODA 子环与 `OODA Scale Scheduler` 已作为正式方法论基线接受
...
7. 状态机主要描述 `R2`、`R3`、`R4` 的业务运行面；`R1` 反射环作为高优先级抢占通道，不等待业务状态机批准。
...
3. `OODA Scale Scheduler` 负责在 `R1` 到 `R4` 之间切换尺度与调度重心，但不替代顶层状态契约。
...
- `主动接近`、`陪伴交互` 更常落在 `R2 执行环`。
- `健康监测`、`用药服务`、`异常升级` 更常由 `R3 任务环` 主导。
- `家庭安全巡护` 承接环境巡检、低打扰巡视、家庭风险发现与必要时的异常上卷。
- 长期提醒优化、习惯学习和服务编排虽然不直接体现在业务主状态枚举里，但由 `R4 关系与服务环` 持续影响这些状态的触发条件和策略参数。
- `保姆协同` 不再作为业务主状态，而作为横切的角色 / 权限 / 任务来源模式，体现在 `world_state_memory`、任务上下文与 `App / 云 / 运营` 协同层中。
...
- 在低打扰前提下完成环境巡检、家庭风险发现、异常候选确认和必要的安全巡护任务

说明：

- 该状态不是传统安防巡逻的简单映射，而是围绕“环境安全 + 家庭秩序 + 低打扰观察”构建的一代安全辅链。
- 进入该状态后，机器人可在授权空间边界内进行环境核查、夜间巡护、门窗与高风险区域观察，以及风险候选事件的本地复核。
- 发现高风险异常时，应立即中断当前巡护流程并进入 `异常升级`。

### 7.8 `保姆协同（横切模式）`

目标：

- 作为角色 / 权限 / 任务来源模式，约束保姆在授权范围内如何触发提醒、到人任务、用药协同、汇报和远程确认

说明：

- 该模式不回答“机器人当前在做什么”，而回答“当前任务由谁发起、谁可确认、谁可查看、谁可中断”。
- 因此它不进入业务主状态枚举，而进入 `world_state_memory`、授权模型、任务上下文和操作审计链。
- 涉及老人本人和子女等高权限角色冲突时，必须进入 `权限冲突待确认`。
...
为支持“交互结束恢复原任务”“低电打断后恢复”“异常升级抢占并在条件满足后恢复”等场景，状态机当前显式引入以下 3 个协同构件：

1. `TaskContextStack`
说明：记录被打断任务的 `task_id`、业务主状态、运动域、打断原因、恢复条件、超时与上下文快照；默认最大深度不超过 `3`。

2. `StateTransitionIntent`
说明：由 `Decision Orchestration` 统一生成的状态切换意图，至少包含目标业务主状态、运动域目标值、资源约束域目标值和原因码，不允许各域各自独立切换。

3. `CompositeStateSnapshot`
说明：记录一次状态切换后的统一快照，用于审计、回放和验证，不再只分别记录若干单轴状态。
