...
文档变更记录：
- v1.7 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：将后台服务 / 人工坐席相关状态从纯后续适配位调整为 `KBT-57` 候选位，未冻结前不进入当前离散决策主链。
- v1.6 | 2026-04-09 | Codex-架构师 | 继续补齐当前主链口径：统一价值排序到“健康管理 > 陪伴交互 > 家庭安全巡护 > 老人看护”，并把穿戴缺数场景的默认补救改写为“本地补采 / 家属确认优先，人工服务仅后续适配”。
- v1.5 | 2026-04-09 | Codex-架构师 | 将业务主状态从 `8` 个压缩为 `6` 个，把 `主动接近 / 用药服务 / 保姆协同` 并入 `执行服务`，并明确当前 V1 只把人工服务相关约束保留为后续适配位。
...
4. 高风险异常、关键安全故障、低电、后续适配位人工接入和权限冲突都视为跨状态中断源。
...
| `执行服务` | 已批准任务的到人执行、提醒、递送、确认和协同执行 | 到人任务、送药任务、协同任务、已批准的异常处理动作 |
...
| `人工服务协同` | 表示人工服务 / 坐席已接入或正在接力 | 当前由 `KBT-57` 承接是否升级为主链，未冻结前不作为必交付 |
...
10. `健康监测 -> 执行服务`
条件：需要到人确认、补采、提醒、递送或其他已批准任务。

11. `健康监测 -> 异常升级`
条件：风险上升到需要升级处置。

12. `执行服务 -> 待机守候`
条件：当前提醒、递送、确认、记录或协同任务完成。

13. `执行服务 -> 异常升级`
条件：用户未响应、状态恶化或执行过程中发现高风险事件。
...
stateDiagram-v2
    ...
    健康监测 --> 待机守候: 无异常
    健康监测 --> 执行服务: 需要到人确认/触发任务
    健康监测 --> 异常升级: 风险升高

    执行服务 --> 待机守候: 任务完成
    执行服务 --> 异常升级: 用户未响应/状态恶化
...
| `后台人工服务接入` | 叠加 `人工服务协同`；该路径当前为 `KBT-57` 候选位，未冻结前不进入当前离散决策主链 |
...
提供 `DecisionContextSnapshot`、当前任务、授权状态和健康基线；人工服务状态是否进入最小快照由 `KBT-57` 决定。

2. `safety_compliance_authorization`
对“到人执行、递送、上报、购药、问诊转接”等动作给出批准、拒绝、降级或待确认结果。

3. `mobility_navigation`
提供到人导航、回充、路径失败和受阻原因。

4. `companion_service_system`
提供家属确认、授权变更、计划任务、事件摘要、记忆治理结果与最小云侧同步结果；人工服务接入由 `KBT-57` 承接是否升级，第三方服务结果当前只保留后续适配位。
...
- `执行服务`、`陪伴交互` 更常落在短周期执行环。
- `健康监测`、`异常升级` 更常由跨步骤任务推进环主导。
- 长期提醒优化、习惯学习和服务编排不会新增业务主状态，但会影响这些状态的触发条件和策略参数。
- 高风险外部触发很多时候以事件形式进入状态机，而不是通过新增状态来表达。

### A3. 个别状态的补充边界

- `健康监测` 中，当穿戴数据新鲜度不足时，应优先触发问诊式补采、`BLE` 外设补采或家属远程确认；人工服务是否前置进入主链由 `KBT-57` 决定。
- `执行服务` 当前边界仍限于提醒取药、送药到人、提醒服药并确认、联系子女，以及已批准的协同执行。
- 涉及照护者协同的任务不表示机器人服从对方的全部命令；高权限冲突时，必须进入 `权限冲突待确认`。
- `异常升级` 当前默认按本体 + App + 最小云主链处理；后台人工服务 / 坐席是否联动立项由 `KBT-57` 决定。
- `回充维护` 必须允许被更高优先级事件打断。
...
| `A5` | 用药高风险事件 | 漏服、重复服、禁忌冲突、紧急药物需求 | 进入 `执行服务` 或 `异常升级` |
