...
| `人工服务协同` | 表示后续适配位中的人工服务已接入或正在接力 | 当前 V1 不作为主链必交付，保留为后续适配位 |
...
| `后台人工服务接入` | 叠加 `人工服务协同`；该路径当前仅保留为后续适配位 |
...
提供 `DecisionContextSnapshot`、当前任务、授权状态和健康基线；人工服务状态当前只保留后续适配位。
...
提供家属确认、授权变更、计划任务、事件摘要、记忆治理结果与最小云侧同步结果；人工服务接入和第三方服务结果当前只保留后续适配位。
...
- `健康监测` 中，当穿戴数据新鲜度不足时，应优先触发问诊式补采、`BLE` 外设补采或家属远程确认；人工服务只保留后续适配位。
- `执行服务` 当前边界仍限于提醒取药、送药到人、提醒服药并确认、联系子女，以及已批准的协同执行。
- 涉及照护者协同的任务不表示机器人服从对方的全部命令；高权限冲突时，必须进入 `权限冲突待确认`。
- `异常升级` 当前默认按本体 + App + 最小云主链处理；后台人工服务仅保留后续适配位。
- `回充维护` 必须允许被更高优先级事件打断。
