...
1. **产品与业务约束**  
   首发场景固定为中国大陆家庭室内养老场景；首发人群为独居老人或子女不在身边的老两口；主价值排序固定为“健康管理 > 陪伴交互 > 老人看护 > 家庭安全巡护”。
...
4. **数据与部署约束**  
   原始视觉、语音和生物特征身份信息必须端侧处理并加密；运动安全与本地执行保护必须端侧闭环；断网不得影响运动与避障安全能力。

5. **工程、商业与项目管理约束**  
   续航目标大于 4 小时，尺寸建议不超过 `50 x 50 x 120 cm`，整机物料成本 `6000 到 8000 元`；系统演进不能破坏“聪明、温暖、精致”的高端产品感；当前阶段必须按 `IPD` 阶段门推进，并争取在 `2026-03-31` 完成产品定义与架构冻结。
...
4. 是否接受一代量产主线采用“纯视觉：双目 + 单目 `3 到 5` 个相机 + 自研深度估计 + 多目几何融合”，并将深度相机 / 激光雷达限制为 `EVT` 前验证加速线。
5. 是否接受当前端侧 / 云侧 / 伴生系统的部署边界。
6. 是否接受“路线 `A`：灵活头部 + 普通躯干与底盘”为当前正式形态提案，并将路线 `B` 保留为 `EVT` 备线。
7. 是否接受“双视角一致性检查机制 + `Body Capability Contract` + 接口稳定性策略”共同构成当前一级接口与治理基线。
8. 是否接受头部仿生研究作为后续 `S1 / S3 / S4 / S5` 的共同架构输入，而不是工业设计附属项。
9. 是否接受 `PDCP` 通过后由模块进入各自架构与总体方案设计，而不再回退重定义系统边界。
10. 是否接受“算力调度器一级化 + 世界状态时间尺度分层 + 分层免疫式安全 + 关系质量评价框架”作为后续总体方案阶段的显式吸收方向，而不重开当前主基线。
11. 是否接受当前需求侧硬约束与 `D1 / D6 / D7` 阻断项作为 `PDCP` 会前必须对齐的管理层输入。
