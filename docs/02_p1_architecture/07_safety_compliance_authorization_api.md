...
2. 门控时要读哪些上下文
3. 门控后会返回哪些标准结果
4. 当前 `V1` 最小主链里，哪些结果会触发确认、降级、故障保护或审计
...
- 后续适配位中的人工服务、第三方平台调用与转接链，统一下沉到文末附录说明，不再与主干混写成当前默认主路径。
...
### 11.5 `manual_service_required`（后续适配位预留）
...
1. 外部下单默认至少返回 `confirmation_required`，并显式标注第三方责任边界。
2. 高风险异常转人工默认至少返回 `manual_service_required`，并显式标注转接目标与审计链。
