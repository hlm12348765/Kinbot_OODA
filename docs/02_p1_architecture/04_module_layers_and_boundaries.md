...
文档变更记录：
- v1.7 | 2026-07-20 | Codex-架构师 | 同步新的 L1 候选拓扑：旧九模块后续向 `A0-A4 + AP1 + F1` 做责任映射，仍不得在 KBT-59 评审前冻结实现边界。
- v1.6 | 2026-07-17 | Codex-架构师 | 增加递归 Agentic 迁移声明：旧九模块仅作为责任来源与迁移映射，不再作为未来软件组织冻结基线。
- v1.5 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：将后台服务 / 人工坐席从纯后续适配位升级为 `KBT-57` 中的 `companion_service_system` 立项联动候选。
- v1.4 | 2026-04-09 | Codex-架构师 | 继续修正当前主链口径：统一价值排序到“健康管理 > 陪伴交互 > 家庭安全巡护 > 老人看护”，并把云侧重服务能力改写为后续适配位，不再写成当前主路径。
- v1.3 | 2026-04-09 | Codex-架构师 | 将 `companion_service_system` 的当前语义收缩为 `App + 最小云`，把坐席、第三方服务和重服务协同降为后续适配位，并同步收紧当前模块边界说明。
- v1.2 | 2026-04-06 | Codex-架构师 | 按 `Phase 4` 口径吸收七实体 `World State` 目标模型，补入 `world_state_memory` 对七实体状态平面的承接说明。
- v1.1 | 2026-04-06 | Codex-架构师 | 按家庭共居智能体革新路线对齐本文，明确 `9` 个一级模块属于三轴总图中的 `WHAT` 轴，不再隐含等同于 `OODA` 分环或总运行时本身。
- v1.0 | 2026-03-08 | Codex-架构师 | 文档创建。

---

> **迁移声明（2026-07-20）**：本文的九模块边界是上一轮软件组织参考，只用于责任追溯与向新架构迁移；不得继续把它当作当前冻结拓扑。当前 L1 首要评审入口为 [16_recursive_agentic_robot_system_architecture.md](16_recursive_agentic_robot_system_architecture.md)，`A0-A4 + AP1 + F1` 精确边界仍在评审。

## 1. 文档目的

本文档记录上一轮总体架构的模块细化结果，当前只作为迁移映射和历史责任边界参考。
...
- 当时总架构上抬为“家庭共居智能体”，运行时改写为“多执行范式基线”
- 当时的 `9` 个一级模块回答三轴总图中的 `WHAT` 轴；现在只用于向 `A0-A4 + AP1 + F1` 候选架构做责任映射
- `World State` 目标模型已收敛为 `7` 类一级实体；这不改变历史映射，但新的能力域划分以 `KBT-59` 评审为准
- 后台服务 / 人工坐席是否与机器人本体产品联动立项，由 `KBT-57` 承接；未冻结前不改变当前 `V1` 已确认主链

说明：

- 首版主价值排序已确认为“健康管理 > 陪伴交互 > 家庭安全巡护 > 老人看护”。
...
1. 它们是稳定能力域，而不是固定时序环节。
2. 它们主要对应家庭共居智能体三轴总图中的 `WHAT` 轴。
3. 离散决策、连续流式、事件驱动和长周期演化属于 `HOW` 轴，不在本文中被重新画成一级模块。
4. `safety_compliance_authorization` 与 `observability_data_governance` 当时保持系统级硬边界地位；新架构分别映射到 A7/A8/F1 候选责任。
...
- 这不意味着 App 和最小云在实现或团队分工上必须合并，只表示它们在一级模块图中属于同一伴生系统域。
- `safety_compliance_authorization` 和 `observability_data_governance` 仍是横切模块，但实现上建议独立建模块而不是散落在各系统里。
- `decision_orchestration` 不等于 `Decide` 本身，`world_state_memory` 也不等于全部 `Orient`；它们只是离散决策主链中最核心的两个能力域。
- 连续流式、事件驱动和长周期演化会调用或影响这些模块，但不会在模块图里各自变成新的一级模块。
...
- 维护统一的 `World State`
- 在当前主线中承接 `Person / CareRelationship / Household / Place / Object / Task / CareEvent` 七实体状态平面
- 维护空间、人员、风险、任务、权限和设备状态
- 维护短期记忆、长期记忆、用户画像和家庭偏好
- 维护健康基线和异常事件历史
- 作为多执行范式共享的状态平面，承接离散主链、事件回写和长周期策略更新
...
- 不直接连接外设驱动
- 不替代专用安全门控
- 不等同于所有认知过程本身，只负责提供稳定、可共享、可审计的状态表达

输入输出：

- 输入：感知事件、导航状态、交互状态、远程确认结果与后续适配位结果
- 输出：决策快照、上下文状态、长期画像、任务上下文

补充说明：

- `world_state_memory` 承接的是七实体目标模型，不意味着当前要新增第 `10` 个一级模块。
- 七实体属于状态平面内部的目标对象结构；`9` 个一级模块继续属于能力域划分。
...
- 在离散决策主链中承担动作提议与业务编排，并与事件驱动、长周期策略更新保持协调
...
- 不直接控制电机
- 不直接接第三方医疗或购药 API
- 不等于全部运行时智能，不替代连续流式策略、事件回写机制或长期学习过程
...
- 伴生系统不能直接控制底盘和执行器
- App 和最小云都不能绕过端侧门控直接下达危险动作
- 伴生系统默认不能持有原始敏感数据
- 医疗、购药、档案和远控能力必须建立显式授权和审计
- 伴生系统对本体的约束应停留在接口、职责和交付面，不应反向锁死本体器件和结构实现
- 后台人工服务 / 坐席当前进入 `KBT-57` 立项联动候选；第三方服务和智能家居仍只保留后续适配位，不进入当前主链

输入输出：

- 输入：机器人状态、告警、授权请求、用户配置
- 输出：用户设置、授权变更、远程确认、计划任务、事件摘要、失败原因、云侧配置
...
1. 这条主链主要描述离散决策范式中的稳定业务主链。
2. 事件驱动范式会从链路中部插入触发和回写。
3. 长周期演化范式主要影响参数、阈值和长期策略，不要求自己变成显式主链节点。
...
基于“健康管理 > 陪伴交互 > 家庭安全巡护 > 老人看护”的排序，首版必须接受以下权衡：
...
5. 家属通知、远程确认、记忆治理和最小审计会比高级巡逻策略更早进入核心路径；在线问诊、购药和配送继续只保留后续适配位。
...
3. 健康事件管线：把信号接入、风险生成、升级链路和后续适配位边界固定下来
...
2. 后台人工服务 / 坐席是否联动立项，以及角色边界、`SLA`、接入链路、非隐私数据回流和 `OPEX`
