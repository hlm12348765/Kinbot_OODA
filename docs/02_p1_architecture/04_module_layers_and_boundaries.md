...
> **迁移声明（2026-07-17）**：本文的九模块边界是上一轮软件组织参考，只用于责任追溯与向新架构迁移；不得继续把它当作当前冻结拓扑。当前 L1 首要评审入口为 [16_recursive_agentic_robot_system_architecture.md](16_recursive_agentic_robot_system_architecture.md)，精确 `F1 + A1-A8` 边界仍在评审。
...
- 当时的 `9` 个一级模块回答三轴总图中的 `WHAT` 轴；现在只用于向 `F1 + A1-A8` 候选架构做责任映射
