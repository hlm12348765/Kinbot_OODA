...
## 10. 附录 A：`Phase 4.5` 战略接口占位

本附录只做战略接口预留，不改写当前 `Phase 1-4` 已冻结结论。

### 10.1 `FleetView` 占位

当前七实体模型继续只描述单家庭 / 单机器人闭环，但在后续平台化方向上，需要为以下能力保留占位：
...
- `FleetView` 不是正式第 `8` 个实体；
- 当前只作为后续平台化视图位说明。

候选字段示意（`Phase 4.5` 增量补强追加，仅作占位，不引入字段冻结）：

| 候选字段 | 含义 | 与现有 7 实体的关系 |
| --- | --- | --- |
| `fleet_state` | 同一机型在多个家庭部署的统计视图 | 不属于任何单一 `Household`，是跨 `Household` 的聚合视图位 |
| `shared_memory` | 跨家庭脱敏后的可共享照护知识片段 | 不属于任何单一 `Person`，需结合附录 B 第 8 条原则候选讨论隐私与生成边界 |
| `cross_household_pattern` | 跨家庭出现的稳定照护模式（例如某种用药后的常见反应） | 不属于任何单一 `CareEvent`，是跨 `CareEvent` 的模式聚合 |

字段命名仅为占位示意，最终命名待 `Phase 4.5 / KBT-54` 收口节点之后再决定。`08_reviews/23` §9 机群能力接入点清单引用本子表作为字段定义来源。

### 10.2 伴生系统对等参与者占位

当前 `WHAT` 轴仍以 `9` 个能力域表达，但在长期平台化方向上，家属 App、云服务、人工坐席与第三方履约不应只被理解为外围集成对象，也应被视为未来对等协同节点。

这一占位当前只服务于战略表达，不改动任何现有接口冻结结论。
...
在 `Phase 4.5` 中，允许补入一个**生成范式（候选）**说明，用于承接：
