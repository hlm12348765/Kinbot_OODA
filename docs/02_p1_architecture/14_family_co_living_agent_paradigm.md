...
文档变更记录：
- v1.5 | 2026-07-17 | Codex-架构师 | 增加递归 Agentic 迁移声明：本文继续作为 Phase 2 背景和决策来路，不再定义当前软件拓扑。
...
> **迁移声明（2026-07-17）**：本文只保留 Phase 2 的母命题和历史决策来路；当前递归 Agentic 方向及 L1 候选架构由 [16_recursive_agentic_robot_system_architecture.md](16_recursive_agentic_robot_system_architecture.md) 承接。
...
3. 做什么：在 Phase 2 当时落回 `9` 个一级能力域与双视角架构；现在由 `KBT-59` 评审新的递归责任域。

## 4. 三轴总图

Phase 2 当时推荐用“三轴框架”表达总图：

1. **照护关系轴（WHY）**：系统为什么采取某个动作，判断标准不只是任务完成率，而是生活连续性、主体性保护、责任关系与低打扰共居。
2. **执行范式轴（HOW）**：系统如何运行，包括离散决策、连续流式、事件驱动和长周期演化。
3. **能力域轴（WHAT）**：系统由哪些稳定能力域承接；当时保留 `9` 个一级模块，现在只作为迁移参考。
