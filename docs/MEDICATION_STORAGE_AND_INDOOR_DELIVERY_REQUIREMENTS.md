...
| `应该有` | 储物记录、交接确认、服药结果记录、家属通知、保姆模式协同、仓门卡滞审计 |
...
1. 与 [docs/HEALTH_EVENT_PIPELINE.md](docs/HEALTH_EVENT_PIPELINE.md) 的关系：本文把其中“计划检查 / 递送执行 / 服药确认 / 结果归档”展开成独立产品链。
2. 与 [docs/SAFETY_COMPLIANCE_AUTHORIZATION_API.md](docs/SAFETY_COMPLIANCE_AUTHORIZATION_API.md) 的关系：开仓、递送、共享结果和通知都必须过统一审批门。
3. 与 [docs/DECISION_STATE_MACHINE.md](docs/DECISION_STATE_MACHINE.md) 的关系：本文主要冻结 `主动接近`、`用药服务` 和 `保姆协同` 里的执行边界。
4. 与 [docs/WORLD_STATE_SCHEMA.md](docs/WORLD_STATE_SCHEMA.md) 的关系：`MedicationAsset`、`Task`、`compartment_policy` 等状态字段要承接本文的装载、交接与审计边界。
5. 与 [docs/HUMAN_SERVICE_AND_TELEMEDICINE_BOUNDARIES.md](docs/HUMAN_SERVICE_AND_TELEMEDICINE_BOUNDARIES.md) 的关系：当递送链中断、身份不确定或需要外部协同时，必须按既定人工服务与第三方责任边界转接。

## 10. 当前提案中的待审点

以下 4 点目前作为 `KBT-10` 的正式评审入口：

1. 是否接受把储药与室内递送要求收敛为 `R1` 到 `R7` 七个能力包。
2. 是否接受“递送 = 到达可交互位置 + 开仓提示 + 用户自取 / 确认”，而不是复杂机械臂交付的一代边界。
3. 是否接受当前的 `必须有 / 应该有 / 可以有` 分层。
4. 是否接受“结构、重量、功耗、外观”与递送能力一起进入同一条工程护栏，而不是只从功能角度定义储物仓。
