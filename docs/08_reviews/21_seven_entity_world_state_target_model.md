...
为承接中国分布式家庭中的多方照护现实，当前允许在 `CareRelationship` 下补入一个**候选**能力：

- `composite_responsible_party`
...
在 `Phase 4.5` 中，允许把 `phase` 的长期演进路径写成候选受控枚举：
...
- 该四阶段枚举当前只作为候选演进路径；
- `V1` 只允许对 `initial -> familiar` 做观测式迁移信号采集，不参与决策。
