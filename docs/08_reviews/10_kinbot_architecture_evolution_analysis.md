...
文档变更记录：
- v1.4 | 2026-03-23 | Codex-架构师 | 吸收 Step41，将“一代 Agent 增强平面”纳入阶段 3 的同代际优化路径，明确长期记忆、技能化、连接器与受控任务编排进入一代正式主线，而“自主创造新技能”继续保留在代际跃迁前的研究线。
- v1.3 | 2026-03-23 | Codex-架构师 | 吸收最新架构收敛决策，补充“从验证 Demo 到一代 PDCP 的五个关键架构变更决策”，明确三芯片功能分层向两芯片硬件收敛、传感器删减、头部多自由度主动观察、单麦阵与底盘基线保留的转译逻辑。
...
### 5.4 从验证 Demo 到一代 `PDCP` 的五个关键架构收敛决策

从验证 Demo 走向一代 `PDCP`，最关键的不是简单删器件，而是把 Demo 的能力验证结论转译成更低表面复杂度、更低成本、更优雅的一代产品架构。

当前已经出现 5 个关键收敛决策：

1. **算力与控制域从三芯片硬件表象收敛为两芯片硬件基线**  
   Demo 证明了“交互 / 感知 / 实时控制”三类职责确实不同；但一代不再保留 `RK3576 + S100Pro + MCU` 三芯片硬件表象，而是收敛为“单主控 SoC + 实时控制 `MCU`”。

2. **传感器从高冗余验证平台收敛为最小纯视觉闭环组合**  
   `4` 固态激光雷达、`4` 超广角、底部超声波、药箱 `ToF` 不再进入一代默认路线；视觉主线进一步收敛为 `1` 组双目 + `2` 个单目。

3. **头部从交互外壳升级为主动观察与拟人表达核心**  
   相机明确集中在头部，头部自由度进一步增加，用“少相机 + 多自由度观察”替代“多固定视角堆叠”。

4. **声学从双麦阵补丁式架构收敛为单麦阵一体化架构**  
   麦克风阵列缩减为 `1` 个并优先布置在头部；扬声器优先放在头部或接近头部的躯干位置，不再默认接受“双麦阵修补式协同”。

5. **本体运动结构从多自由度验证平台收敛为“头部灵活 + 躯干克制 + 底盘稳健”**  
   躯干不再追求复杂自由度；底盘可评估全向方案，但产品基线继续保留“两轮差速 + 全向轮”。

```mermaid
flowchart LR
    A["验证 Demo
    三芯片 + 多传感器 + 多补丁"] --> B["一代 PDCP
    两芯片硬件基线 + 最小纯视觉闭环"]
    B --> C["头部优先
    多自由度主动观察"]
    B --> D["单麦阵一体化声学"]
...
1. 一代架构不是在“保留 Demo 的高配优势”，而是在保留真正有价值的能力结构。
2. Demo 的“三层功能分工”继续保留，但 Demo 的“三芯片硬件表象”不再保留。
3. 一代的技术突破点从“堆传感器”转向“纯视觉 + 主动观察 + 系统调度”。
...
除前述 5 类演进外，基于 `Step41`，阶段 3 还应增加第 6 类同代际优化：

### 6.8 演进六：从离散 AI 能力走向一代 Agent 增强平面

这一步不等于引入新的超级模块，也不等于把 `OpenClaw / Claude Code / Codex` 这类外部工具直接写成产品运行时模块。

更准确的说法是：

1. 一代正式接受“长期记忆、技能化、连接器、受控任务编排”进入主线。
2. 这些能力必须压缩在现有模块边界内实现，而不是再长出新的一级模块。
3. 这仍属于同代际优化，因为它强化的是一代系统的可组合性和可扩展性，而没有改变 Kinbot 与家庭成员的基本共存样式。

这一演进的关键价值在于：

1. 让一代开始具备可治理的长期记忆，而不是只剩短期上下文。
2. 让能力以“技能”而不是零散逻辑的方式组织。
3. 让 App、云、手机、智能家居和第三方服务的连接边界被统一抽象。
4. 让 `OODA` 主环获得更强的任务编排能力，同时不放弃安全、合规和审计边界。

这一步的边界也必须写清：

1. 一代不把“自主创造新技能并自动上线”写成正式承诺。
2. 一代允许固定技能和组合技能，但候选新技能仍停留在研究线。
3. `Predict / Learn` 仍然是跨环能力，不被简单改写为新的主环串行步骤。
...
| Agent 能力 | 离散增强能力 | 形成不新增一级模块的 Agent 增强平面 |
...
1. 一代的长期记忆与技能化，只是后续经验学习与技能演化的前提层。
2. 真正“自主创造新技能”的能力更适合作为阶段 4 的研究与二代候选方向，而不是阶段 3 的一代正式承诺。
