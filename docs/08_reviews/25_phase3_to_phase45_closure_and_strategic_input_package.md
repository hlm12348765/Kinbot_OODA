...
文档变更记录：
- v1.1 | 2026-04-09 | Codex-架构师 | 将当前未冻结项收紧为“摘要 + Linear 指针”治理，明确由 `KBT-32 / KBT-33 / KBT-55 / KBT-56` 承接，不再把活跃总包写成无 owner 的 provisional 堆积载体。
...
以下内容仍保留为 `provisional`，但当前统一按“摘要 + Linear issue 指针”治理：

| 项目 | 当前摘要 | Linear 承接 | 冻结条件 | 目标阶段门 | 当前处理结果 |
| --- | --- | --- | --- | --- | --- |
| 七实体完整字段表 | 当前只冻结最小激活子集，不提前展开全量字段表 | `KBT-32` | `S2 / S4 / S5` 下游对象、接口与验证清单稳定 | `KBT-32` | 保持 `provisional` |
| `CareRelationship` 关系质量维度 | 当前不提前决定连续型评分还是分段等级的最终形态 | `KBT-32` | 关系演化的最小观测口径、消费方与验证方式明确，且不反向扩张主线 | `KBT-32` | 保持 `provisional`，后续可按真实数据再转 research |
| `CareEvent` 最终事件族全集 | 当前只冻结边界规则，不提前冻结全集 | `KBT-32` | 事件边界、审批依赖与 `S2 / S4 / S5` 验证职责明确 | `KBT-32` | 保持 `provisional` |
| proto 与下游接口 `breaking change` 策略 | 当前仍缺 owner、版本号、变更门与迁移顺序的完整治理表达 | `KBT-33` | 接口 owner、版本号、变更门和迁移规则明确 | `KBT-33` | 保持 `provisional` |
| 远程在场感、关系演化、惊奇时刻的最终验收门槛 | 当前只冻结三条战略验证对象，不提前冻结量化阈值 | `KBT-55` | 真实进入 `Phase 5` 实机验证并具备有效样本 | `KBT-55 / G5` | 保持 `provisional` |

统一治理母单为 `KBT-56`，用于维护活跃 `provisional` 的字段模板、owner 与 orphan 清账节奏。
