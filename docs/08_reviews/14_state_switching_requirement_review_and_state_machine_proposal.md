...
1. `巡逻状态` 在方案 C 中承担一代家庭安全巡护主链，而不是继续停留在泛巡检语义。
2. `保姆协同` 不再作为业务主状态。
3. `找物状态` 不再作为顶层业务主状态，转为 `L2` 叶子服务能力。

#### 8.2.1 与当前主线术语映射

为保持本评审文档内部命名与原始方案、方案 A、方案 B 完全一致，方案 C 继续沿用原始状态名；与当前主线术语的关系如下：

| 方案 C 用词 | 当前主线常用表达 | 说明 |
| --- | --- | --- |
| `交互状态` | `陪伴交互` | 方案评审阶段沿用产品经理原始状态名 |
...
方案 C 中，交互不再独立成正交域，而是作为一棵可复用的内部行为树，被以下业务主状态按需调用：
