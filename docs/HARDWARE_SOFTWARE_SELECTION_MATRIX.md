...
- 结合 `Step21` 与 `Step22` 的审阅意见，当前滚动修正已转入 [docs/COST_STRUCTURE_AND_TECH_DOWNPATH.md](docs/COST_STRUCTURE_AND_TECH_DOWNPATH.md) 继续细化。
...
6. 任何试图压低 `C4` 的方案，都必须同步展示其对 `C1 / C2 / C5` 的新增成本与新增风险。
...
1. 是否接受 `Step22` 中对 `C1 / C3 / C5` 的方向性修正正式进入滚动成本基线。
2. 是否接受 `C2 / C6 / C7` 继续作为一代显式降本主战场。
3. 是否接受 `C5` 当前只给工作区间、不宣称已经冻结成本基线。
4. 是否接受 `C4` 的低传感路线必须显式评估其对 `C1 / C2 / C5` 的成本转移。
5. 是否接受把“功耗预算与能效控制策略”拆成新的后续 issue 持续收敛。
