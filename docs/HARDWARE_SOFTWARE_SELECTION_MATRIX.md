...
| `C5 电池、电源与热设计` | `650 到 900` | 电池包、电源板、散热与供电保护 |
...
- 结合 `Step21` 的第二轮审阅意见，当前滚动修正已转入 [docs/COST_STRUCTURE_AND_TECH_DOWNPATH.md](docs/COST_STRUCTURE_AND_TECH_DOWNPATH.md) 继续细化。

### 3.2 `Step21` 后的滚动修正

基于你在 `Step21` 中给出的方向性判断，当前先做 3 条滚动修正：

1. `C1 端侧算力与存储` 当前按“较上一轮提案上修约 `300` 元”继续监测。
2. `C3 音频与交互器件` 当前按“音频较便宜、显示屏更贵且与外观耦合”继续下修评估。
3. `C5 电池、电源与热设计` 当前按“续航与热设计压力上升”继续上修评估。
...
4. 只要高算力专项路线持续把 `C1 端侧算力与存储` 压到 `1700 元` 以上，就必须同步给出其他成本桶的回收方案；若无法回收，则该路线不得直接成为一代量产定型。
...
1. 是否接受把 `Step21` 中对 `C1 / C3 / C5` 的方向性修正，升级为滚动成本基线的一部分。
2. 是否接受 `C2 / C6 / C7` 作为一代显式降本主战场。
3. 是否接受 `C1` 的近期目标以“控涨”为主，而不是假设它会立即自然下降。
4. 是否接受 `C5` 当前主要目标也是“控制涨幅”，而不是承担主要降本责任。
5. 是否接受将 BOM 成本结构和技术降本路线拆成独立 issue 持续跟踪。
