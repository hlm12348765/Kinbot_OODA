...
文档变更记录：
- v1.4 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：将后台服务 / 人工坐席联动立项、非隐私数据回流、`10000` 元 BOM、`29999` 元定价、首批 `10000` 台和租售并行纳入 `KBT-57` 待拆解门控，不覆盖当前 `G5` 已冻结标准。
...
4. 若 `KBT-57` 后续确认后台服务 / 人工坐席与机器人本体联动立项，则服务组织、坐席 `SLA`、非隐私数据回流和 `OPEX` 模型也必须进入闭环方案。
...
| `M2 整机平台与 BOM 冻结` | 结构、底盘、传感器、算力、`BOM`、售价监控区间和功耗主线进入受控状态；若采用 `KBT-57` 的 `10000` 元 BOM 分支，必须给出与当前 `5000 到 6000` 元基线的对照 | 不再有会推倒整机平台的技术漂移 |
...
| `M6 试点与服务运营准备` | `100 台 / 100 户 / 1 个月` 级别试点方案、App、云、坐席、售后和升级链路准备完成；若坐席联动立项成立，需补齐服务时段、响应 `SLA`、排班、质检、成本和数据回流闭环 | 小批量试点随时可执行 |
| `M7 发布与商业准备` | 发布准备、交付、售后、物料、口径和质量监控方案完成；`KBT-57` 中的 `29999` 元定价、首批 `10000` 台、渠道让利压缩和租售并行只能在商业模型闭环后进入发布口径 | 随时可以开发布会并承接试点与交付 |
...
若 `KBT-57` 被确认进入主线，`服务 / 坐席联动立项与商业模式拆解` 应并入第 `2 / 6 / 7` 类证据包，并在第 `8` 类战略验证证据包中补充价值证据。
...
4. 服务 / 坐席联动立项价值：坐席介入能够形成更好的服务闭环、问题归因、产品改进或数据闭环，而不是长期替本体和伴生系统补失败
...
8. 直接把 `10000` 元 BOM、`29999` 元定价、首批 `10000` 台或租售并行写成发布口径，但没有完成对应的毛利、渠道、交付、售后、服务和资金占用测算。
