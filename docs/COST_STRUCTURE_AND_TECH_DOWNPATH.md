...
结合 `Step21` 的第二轮审阅意见，当前建议把一代整机的滚动成本基线收敛为 7 个成本桶。
...
| `C4 底盘与运动系统` | `1450` | `1200 到 1700` | 稳定 | 自研底盘决定可靠性下限，不适合被激进压价 |
| `C5 电池、电源与热设计` | `800` | `650 到 900` | 上行 | 端侧算力变重、续航 `>4h`、散热余量需求上升，当前较上一轮提案上修 |
...
- `C1 + C5` 是当前最强的上行组合压力。
- 如果 `C1` 最终继续突破 `1700`，或者 `C5` 接近 `900` 上沿，则必须由 `C2 / C3 / C6 / C7` 释放成本空间来回收。
...
| `C4` | 中高 | 低到中 | 不应作为当前主要降本战场 | 只能做温和优化 |
| `C5` | 高 | 低 | 当前核心目标是控制上涨 | 中期只能靠整机能效改善释放少量空间 |
...
1. 在 Linear 中单独创建“整机 BOM 成本结构与降本路线” issue，持续跟踪成本滚动变化。
2. 在 `G2` 前形成首版“滚动 BOM 看板”，每月评审一次 `C1 / C5` 上行压力和 `C2 / C6 / C7` 的回收进度。
3. 端侧算力专项评估必须同时带上“模型压缩与量化结果”，不能只比较裸芯片规格。
4. 工业设计、结构、声学、热设计和产测必须一起进入成本收敛讨论，不允许只由采购端单独压价。
