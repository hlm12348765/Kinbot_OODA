...
以下 4 点目前作为 `KBT-14` 的正式评审入口：

1. 是否接受把 `UWB` 的一代角色冻结为“主线不进入、增强线待样品验证、生命体征继续保留研究观察线”。
2. 是否接受“粗定位可以评估、但不进入主导航硬依赖”这一边界。
3. 是否接受“活动状态判断可以做样品验证，但在验证前不写入一代标准能力承诺”。
4. 是否接受当前的 `S1` 到 `S5` 样品验证门，作为后续导入 `docs/REQUIREMENTS.MD` 的最小测试框架。
