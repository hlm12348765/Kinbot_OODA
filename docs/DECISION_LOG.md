...
| ID | 问题 | 为什么关键 | 状态 |
| --- | --- | --- | --- |
| Q-001 | 陪伴交互的人设、主动发起边界和打断礼仪到底如何定义 | 决定交互策略、状态机和授权边界 | open |
| Q-002 | 陪伴交互中的长期记忆、隐私共享和多角色上下文边界如何定义 | 决定记忆系统、隐私治理和角色模型 | open |
| Q-003 | 白天与夜间的主动靠近、提醒、闲聊、追问和停止条件如何量化 | 决定主动交互触发器和夜间静默策略细化 | open |
| Q-004 | 一代安全保障优先覆盖哪些家庭危险源与空间风险 | 决定风险事件模型、传感器优先级和验证范围 | open |
| Q-005 | 低电量、定位异常、传感器失效、断网和储物仓卡滞时分别如何降级 | 决定 fail-safe 设计和恢复路径 | open |
| Q-006 | 样机到量产阶段，哪些本体能力必须保留，哪些能力和硬件配置可以重构 | 决定技术路线、BOM 和工程规划 | open |

## 5. 下一轮优先向用户确认的问题

建议下一轮优先从陪伴交互和安全保障两条线继续澄清，避免架构重心继续偏向健康单线：

1. 你希望一代机器人的陪伴“人设”更偏稳重照护型、亲切家人型、幽默搭子型，还是允许有限可配置？哪些表达方式绝对不能越界？
2. 白天哪些情境下一期允许机器人主动发起交互：长时间沉默、久坐久卧、情绪低落、到点提醒、用户回家问候，分别哪些必须做？
3. 哪些用户信息允许做长期记忆：偏好、习惯、对话片段、家庭事件、健康历史；哪些内容绝不长期保留或绝不共享给子女/保姆？
4. 一代安全保障优先覆盖哪些家庭危险：跌倒、久卧不起、卫生间滑倒、厨房明火、门口/楼梯风险、夜间离床、陌生人闯入，怎么排序？
5. 当低电量、定位异常、传感器失效、网络断开、储物仓卡滞时，机器人分别应该进入什么降级状态，哪些动作必须立即停用？
6. 从当前样机出发，你最希望保留的三项 Demo 能力和最愿意重构的三项能力分别是什么？
