...
1. 后台人工服务与在线问诊协同边界，当前是否优先先于储药与室内递送要求继续清账？
2. 储药与室内递送要求里，哪些必须被冻结到“核心状态与接口冻结”里程碑，哪些可推迟到工程化阶段？
3. 一期穿戴设备兼容范围与数据字段，是否应先于 UWB 技术成熟度评估完成？
4. UWB 评估是否继续作为技术路线评估里的观察线 issue，而不阻塞当前主线？
5. 是否接受把 `KBT-16` 保留为已起草提案，但在前置里程碑未清账前不作为当前主评审项？
