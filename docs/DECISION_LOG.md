...
| A-067 | 当前三类阻断项默认收敛为：伴生系统与人工运营闭环、整机平台与成本收敛、工程化与 NPI 基线；其中 `D1` 与 `D7` 已获确认，`D6` 需结合阻断理由进一步冻结 | provisional | `Step13` 对 `KBT-18` 的审阅意见 |
...
| Q-006 | 是否接受当前为 `D6` 给出的阻断理由，并据此将其正式冻结为阻断项 | 决定 `KBT-18` 是否可以完全收口并转入工程化主线 | open |
| Q-007 | 上市后质量、人工服务、版本治理和下一代回灌机制要做到什么深度 | 决定全生命周期后段的组织和系统投入 | open |

## 5. 下一轮优先向用户确认的问题

建议下一轮优先把 `KBT-18` 收口和 `KBT-24` 基线确认下来：

1. 你是否接受当前为 `D6` 给出的阻断理由，并据此将其正式冻结为阻断项？
2. 你是否接受按 7 个工程化与 `NPI` 基线包推进，而不是按部门职能推进？
3. 你是否接受把 `E4 伴生系统与服务运营基线` 放在 `KBT-24` 的第一优先级？
4. 你是否接受把“储物仓机构重构”和“整机外观形态重构”明确列入 `E2` 的既定动作？
5. 你是否接受当前给出的 `G2` 七项放行条件？
6. 低电量、定位异常、关键传感器失效这 3 类故障，你希望分别用什么阈值切换到“受限继续 / 挂起任务 / 故障保护”？
...
| 2026-03-08 | D-070 | Demo 缺口主文档 | 新增 `docs/DEMO_TO_MASS_PRODUCTION_GAPS.md`，将当前样机到量产预备的差距收敛为 7 个能力域、3 类阻断项和一份默认优先级排序 | provisional |
...
| 2026-03-08 | D-074 | 工程化与 NPI 主文档 | 新增 `docs/ENGINEERING_NPI_BASELINE.md`，将 `KBT-24` 收敛为 7 个工程化与 `NPI` 基线包、`G2` 七项放行条件和一份默认优先级排序 | provisional |
