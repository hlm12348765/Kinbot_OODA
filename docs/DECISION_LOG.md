...
1. 你是否接受 `KBT-8` 当前增加的 `fault_protection_required` 返回结果？
2. 你是否接受 `KBT-8` 把 `A1-A7` / `F1-F7` 作为审批上下文的一部分输入？
3. 你是否接受 `KBT-8` 当前的原因码骨架，尤其是 `ANOMALY_*` 与 `FAULT_*` 前缀？
...
| 2026-03-08 | D-087 | `KBT-8` 接口强化提案 | 当前建议审批接口显式支持 `fault_protection_required` 返回结果，并支持 `state_transition_hint` 把关键安全故障直接映射到 `故障保护` | provisional |
