...
| Q-012 | `KBT-12` 中 `必须有 / 应该有 / 可以有` 范围与 `P / W / B` 放行规则是否需要进一步量化 | 决定 `MVP` 验证计划能否从结构化计划进入执行计划 | open |

## 5. 下一轮优先向用户确认的问题

建议下一轮优先按 `KBT-12` 当前提案继续清账：

1. 是否接受当前的 `必须有 / 应该有 / 可以有` 一代 `MVP` 范围划分？
2. 是否接受 `V1-V7` 这 `7` 个一级验证域？
3. 是否接受当前的 `100 台 / 100 户 / 1 个月` 四层试点执行框架？
4. 是否接受 `P / W / B` 三类放行与阻断规则？
5. 对 `Q-012`，你是否希望下一轮直接把 `P / W / B` 对应到更量化的执行阈值？
6. 在 `KBT-12` 之后，是否回到 `Q-001 / Q-005` 这类量化阈值问题？
