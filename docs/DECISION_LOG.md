...
| A-075 | 每一阶段都要检查系统各实体元素的组合，判断整机与服务是否仍然呈现“聪明、温暖、精致”的高端产品感，并能支撑约 `30000 元` 价格带 | confirmed | `docs/REQUIREMENTS.MD` 对 Codex 的要求第 10 条 |
...
| Q-006 | 如何冻结 `C5` 的工作基线，并在 `C4` 低传感路线下显式记账 `C1 / C2 / C5` 的成本转移 | 决定一代成本是否既可收敛，又不会因为“压低底盘传感器”而隐性放大算力、视觉和功耗成本 | open |
...
1. `C5` 的工作基线，是否按“静默待机 / 陪伴交互 / 高频运动”三类工况分别定义？
2. 低电量、定位异常、关键传感器失效这 3 类故障，你希望分别用什么阈值切换到“受限继续 / 挂起任务 / 故障保护”？
3. 智能家居一期优先接入哪些设备类型和协议？
4. 家属 App 允许下发哪些远程控制指令，哪些必须禁止？
5. 卫生间、卧室、厨房、入户门是否需要细化成正式的空间规则表？
6. 对 `6000 到 8000 元` BOM 约束下的每次显著降本动作，是否都要附带“高端产品感知检查表”作为阶段门输入？
...
| 2026-03-09 | D-106 | 高端产品感知护栏 | 以后每一阶段都必须同时检查系统各实体组合是否仍然呈现“聪明、温暖、精致”的高端产品感，并能支撑约 `30000 元` 价格带，不能只以 `BOM` 收敛作为通过条件 | confirmed |
