...
建议下一轮优先按 `KBT-31` 当前补充提案继续清账：

1. 是否接受“本体 `6` 个实体域 + 运行时 `9` 个一级模块”的双视角总体架构基线？
2. 是否接受把机器人本体显式分解为“计算与控制核心、移动底盘与运动执行、环境与人体感知组件、交互表达组件、电池电源热与本体安全、储物仓结构与外观”这 `6` 个实体域？
3. 是否接受补入 `Body Capability Contract` 作为一级接口面的一部分？
4. 是否接受在补入本体能力接口面后，当前一级接口面足以支撑模块并行设计？
5. 是否接受 `PDCP` 通过后由模块进入各自架构与总体方案设计，而不再回退重定义系统边界？
6. 是否接受 `KBT-32` 在 `KBT-31` 关闭前暂不进入主评审？
