...
| Q-006 | 如何在 `C1` 上行、`C5` 上行的情况下，通过 `C2 / C3 / C6 / C7` 的技术与工程改进，把整机 BOM 稳定压在 `6000 到 8000` 元区间内 | 决定一代量产成本是否真正可收敛，而不是只在纸面上成立 | open |
...
1. 你是否接受当前滚动成本基线里 `C1` 上修、`C3` 下修、`C5` 上修的方向性判断？
2. 你是否接受 `C2 / C6 / C7` 作为一代显式降本主战场？
3. 你是否接受 `C1` 当前以“控涨”为近期目标，而不是假设它会立即自然降价？
4. 你是否接受 `C5` 当前主要负责控制上涨，而不承担主要降本责任？
