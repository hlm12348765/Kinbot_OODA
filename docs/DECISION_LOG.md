...
建议下一轮优先按 `KBT-13` 当前提案继续清账：

1. 是否接受“机器人本地服务 -> 客服运营坐席 -> 医疗专业主体 / 第三方履约”的 4 层接力结构？
2. 是否接受“结构化上下文优先、音频优先、默认不启视频”的一代媒体策略？
3. 是否接受当前的角色责任切分，尤其是客服不做诊断、医生平台负责专业结论、平台负责履约结果？
4. 是否接受当前的接入时效基线，尤其是“客服 `3 分钟` 响应、医疗专业主体 `10 分钟` 内给出接单结果”的提案？
5. 在 `KBT-13` 评审完成后，`KBT-10` 是否继续作为下一个核心状态与接口冻结项？
6. 对高风险异常下的临时音频开放，你更倾向“事件授权默认允许”还是“仍需家属或用户二次确认”？
