...
建议下一轮优先按 `KBT-10` 当前提案继续清账：

1. 是否接受把储药与室内递送要求收敛为 `R1` 到 `R7` 七个能力包？
2. 是否接受“递送 = 到达可交互位置 + 开仓提示 + 用户自取 / 确认”，而不是复杂机械臂交付的一代边界？
3. 是否接受当前的 `必须有 / 应该有 / 可以有` 分层？
4. 是否接受“结构、重量、功耗、外观”与递送能力一起进入同一条工程护栏？
5. 在 `KBT-10` 之后，是否继续按顺序推进 `KBT-15` 而不是 `KBT-14`？
6. 对 `Q-008` 人工服务经济性问题，你更倾向后续在运营阶段单独冻结，还是提前在产品定义阶段补一条服务可用性基线？
