...
| Q-006 | 当前 Demo 到量产预备之间，哪些能力缺口必须优先补齐 | 决定资源投放顺序和后续技术选型重心 | open |
| Q-007 | 上市后质量、人工服务、版本治理和下一代回灌机制要做到什么深度 | 决定全生命周期后段的组织和系统投入 | open |

## 5. 下一轮优先向用户确认的问题

建议下一轮优先把健康事件量化阈值、伴生系统远控边界、Demo 到量产缺口和上市后回灌补齐：

1. 对久卧不起、疑似摔倒、用户自述不适、生命体征异常这 4 类健康事件，一代准备如何定义 `L1` 到 `L4` 的风险阈值？
2. 家属 App 允许下发哪些远程控制指令，例如发起通话、远程播报、查看摘要、取消提醒、远程确认，哪些指令必须禁用？
3. 智能家居一期优先想接哪些设备：烟雾报警、燃气报警、水浸、门磁、窗磁、人体传感器，怎么排序？
4. 卫生间、卧室、厨房、入户门这 4 类空间，是否需要下一轮冻结成正式的规则表？如果需要，哪些动作必须显式列入禁用或例外？
5. 低电量、定位异常、关键传感器失效这 3 类故障，你希望分别用什么阈值切换到“受限继续 / 挂起任务 / 故障保护”？
6. 从当前样机 Demo 到 2026-12-31 量产预备，你认为必须优先补齐的 5 个能力缺口是什么？
7. 上市后你最看重哪 4 类闭环：质量问题闭环、人工服务闭环、运营数据闭环、OTA 版本闭环、下一代需求回灌，怎么排序？
