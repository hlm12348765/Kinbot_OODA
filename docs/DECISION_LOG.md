...
| Q-006 | 一代端侧算力主线最终应在 `RB3 Gen2/QCS6490` 与 `Orin NX` 中如何收敛 | 决定算法迁移、热设计、BOM 和供应链主路线 | open |
| Q-007 | 上市后质量、人工服务、版本治理和下一代回灌机制要做到什么深度 | 决定全生命周期后段的组织和系统投入 | open |

## 5. 下一轮优先向用户确认的问题

建议下一轮优先把软硬件选型和故障阈值收敛下来：

1. 你是否接受把端侧算力主线收敛为 `RB3 Gen2/QCS6490` 与 `Orin NX` 双路线评估？
2. 你是否接受把 `RGB` 简配置 + 自研深度估计作为量产主线，把立体深度模块仅保留为 Alpha / EVT 验证加速路线？
3. 你是否接受继续保持“自研麦阵 + 自研底盘 + 外采穿戴生态”的总体组合？
4. 你是否接受把 `RK3588` 定位为成本与供应备线，而不是一代默认主线？
5. 低电量、定位异常、关键传感器失效这 3 类故障，你希望分别用什么阈值切换到“受限继续 / 挂起任务 / 故障保护”？
6. 智能家居一期优先接入哪些设备类型和协议？
