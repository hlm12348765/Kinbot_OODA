...
| --- | --- | --- | --- | --- |
| `8GB RAM + 64GB Flash` | 约 `1035 元` | 约 `94%` | 约 `61%` | 仍可作为量产默认线，但已经明显压缩 `SoC` 与外围空间 |
...
- 在当前价格口径下，`RAM` 已经成为 `C1` 的核心矛盾：`8GB + 64GB` 仍可勉强讨论量产，`12GB + 64GB` 已进入边界区，`16GB + 64GB` 则仅 `RAM + Flash` 就超过 `C1` 上限。
...
3. `C1` 以“模型压缩 + 板级集成 + 调度优化”做中期降本，但短期目标先定义为锁住量产默认内存线，避免继续被 `RAM` 容量击穿。
4. 当前建议把 `8GB RAM + 64GB Flash` 定义为量产默认线，把 `12GB + 64GB` 定义为边界验证线，把 `16GB + 64GB` 及以上定义为前瞻验证线或未来 `Pro SKU` 候选。
