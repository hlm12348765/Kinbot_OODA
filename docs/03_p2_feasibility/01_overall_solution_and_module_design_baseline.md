...
6. 量产预备、`MVP`、发布与交付等后置文档，只作为远期约束，不反向抢占当前主线
...
6. 由 `VLN` 专家线程承接 `P1 / P2 / P3` 任务模板、结构化动作输出和 `TTFT / TPS` 目标表，并把结果回写到 `S1 / S4 / S7`
...
6. 是否接受把 `TTFT / TPS` 驱动的端侧算力需求模型和 `VLN` 专家线程量化结果，正式纳入 `S1 / S4 / S7` 的共同输入。
