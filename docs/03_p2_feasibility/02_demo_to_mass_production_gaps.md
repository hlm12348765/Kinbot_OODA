...
文档变更记录：
- v1.1 | 2026-04-06 | Codex-架构师 | 按 `Phase 2` 口径重写样机缺口分析的运行时表达，明确当前缺口应对应多执行范式、共享状态平面和长周期关系面，而不再把旧 `OODA` 总图直接投射到量产预备差距表。
...
5. 当前总架构已升级为家庭共居智能体，运行时采用多执行范式；`OODA / R1-R4` 继续作为离散决策范式保留，但不再承担唯一总架构角色。
...
| 维度 | 当前判断 | 主要问题 |
| --- | --- | --- |
| 本体端侧安全与执行面 | 已有基础，但未证明量产级稳健性 | 需要进一步收敛运动安全、局部避障、低照执行与硬停底线 |
| 离散决策业务面 | 已有局部能力，但执行稳定性不够 | 导航反应慢、行走犹豫、身份效果弱、执行监督不足 |
| 健康 / 用药 / 升级业务管线 | 已有 Demo，但仍偏链路演示 | 风险分级、人工服务、家属联动、审计和远控边界尚未产品化 |
| 长周期关系与服务面 | 基本空白或仅有零散能力 | 长期记忆、习惯学习、提醒优化、服务编排尚未形成正式产品层 |


结论：

- 当前样机不是完全没有运行时能力，而是端侧执行、离散决策和健康业务管线有粗放基础，长周期关系与服务面明显偏弱。
- 多执行范式与共享状态平面虽然已在架构层冻结，但尚未看到与当前样机能力显式对齐的产品化落点。
...
| `D2 运动安全与导航执行`    | 已有导航闭环，但反应慢、行走犹豫     | 稳定、可预测、可验证的运动安全与局部执行         | `关键收敛`    | `关键` |
| `D3 身份、多模态交互与离散执行业务面` | 麦阵和自然语音较成熟，身份效果弱     | 稳定的身份识别、多角色绑定、执行监督和低扰动交互     | `关键收敛`    | `关键` |
| `D4 健康、用药与异常升级`      | 已有紧急给药 Demo 和问诊能力    | 形成完整的健康事件、用药、升级和审计闭环         | `产品化重构`   | `关键` |
| `D5 长周期关系与服务闭环`      | 长期服务能力弱              | 形成记忆、习惯、提醒优化和服务编排闭环          | `从 0 到 1` | `关键` |
...
- 端侧硬停、限速、避障和局部执行必须足够稳定，能作为量产底线。
- 到人确认、局部导航和仓门执行要可预测、可复现、可验证。
- 高层语义能力不得被误当成底盘级安全兜底。

主要缺口：

1. 当前导航链更像“能跑通”，还不是“可交付”。
2. 本体端侧执行面、离散决策业务面和健康业务管线的分层虽然在架构上冻结了，但尚未映射为可验证的运行栈能力。
3. 执行范式协调还没有体现到导航执行策略的产品化行为上。

建议动作：

- 把端侧执行、离散决策和健康业务管线对导航的职责拆成正式验证项。
- 先解决“反应慢、犹豫”这类直接破坏体验和安全信心的问题。
- 把 VLN 继续限制在语义策略层，不让其挤压本地执行稳定性。

### 6.3 `D3 身份、多模态交互与离散执行业务面`
...
- 主动靠近、提醒、打断、到人确认等离散交互场景要低扰动、可解释、可回退。
...
3. 离散执行业务面的执行监督能力还未显式产品化。

建议动作：

- 把身份、角色、授权、记忆可见性合并成同一个产品化闭环设计。
- 用高频场景先验证离散执行业务面，例如到人提醒、服药确认、家属远程确认。
...
1. 目前偏“链路演示”，还不是“产品闭环”。
2. 穿戴、BLE 外设、问诊补采、人工服务和家属联动还没有真正收束到同一个共享状态平面与离散业务状态面。
3. 缺少量化阈值、默认升级策略和远控边界。

建议动作：

- 把健康与用药闭环视为核心业务管线，不再按零散功能拆开推进。
...
- 长周期关系与服务面虽已进入正式架构，但尚无对应的样机级产品闭环证据。
