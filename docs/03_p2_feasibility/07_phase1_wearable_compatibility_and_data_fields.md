...
1. 与 [docs/02_p1_architecture/10_health_event_pipeline_and_escalation.md](../02_p1_architecture/10_health_event_pipeline_and_escalation.md) 的关系：本文冻结信号源优先级、字段和新鲜度，作为健康事件管线的稳定输入。
2. 与 [docs/02_p1_architecture/05_world_state_schema.md](../02_p1_architecture/05_world_state_schema.md) 的关系：`device_binding_refs`、`vital_signal_sources`、`wearable_freshness_state` 等字段承接本文定义。
3. 与 [docs/02_p1_architecture/07_safety_compliance_authorization_api.md](../02_p1_architecture/07_safety_compliance_authorization_api.md) 的关系：数据新鲜度不足时，审批层必须支持补采、复核和降级。
4. 与 [docs/03_p2_feasibility/04_hardware_software_selection_matrix.md](04_hardware_software_selection_matrix.md) 的关系：穿戴与家庭设备生态只通过接口约束本体，不反向锁死本体主控路线。
...
- 用户近期将拿到 `UWB` 外设样品；样品测试结果将作为后续 `KBT-14` 和 `docs/00_governance/00_requirements.md` 的增量输入。
