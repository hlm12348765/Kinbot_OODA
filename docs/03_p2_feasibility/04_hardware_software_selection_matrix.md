...
文档变更记录：
- v1.5 | 2026-04-21 | Codex-架构师 | 吸收董事长汇报反馈：保留 `5000 到 6000` 元当前 BOM 基线，同时新增 `KBT-57` 的 `10000` 元 BOM 战略分支，不把硬件配置过度压低当成默认优化目标。
- v1.4 | 2026-04-14 | Codex-架构师 | 吸收 `Step 50` 的头部方案判断：默认主线切到“紧凑轻量头部 + 躯干内容屏”，并将高集成度带屏头明确为头颈高速链路阶段门不过线时的 `Plan B`。
- v1.3 | 2026-04-09 | Codex-架构师 | 吸收 `Step 48` 的资源线调整：将默认量产线更新为 `12GB RAM + 32GB Flash`，并把 `8GB + 64GB` 下调为更激进的成本压缩讨论线。
- v1.2 | 2026-03-20 | Codex-硬件专家 | 吸收最新内存与存储价格口径，补入 `C1` 子桶拆分、`8GB / 12GB / 16GB+` 内存路线门槛与量产/前瞻分层判断。
- v1.1 | 2026-03-17 | Codex-架构师 | 吸收 Step36，更新 BOM 目标、纯视觉对比基线与“不回退主动传感”的选型纪律。
...
在既定产品目标、成本约束和架构基线下，Kinbot 一代的软硬件路线应该如何收敛，哪些路线适合作为量产主线，哪些路线只适合作为研发对比基线，哪些路线只保留为前瞻观察。
...
2. 当前已冻结整机物料成本目标仍为 `5000 到 6000 元人民币`；董事长反馈中的 `10000` 元 BOM 仅作为 `KBT-57` 待拆解战略分支，不能直接改写本文当前主线成本桶。
...
8. 深度相机和激光雷达只作为研发阶段的对比基线与真值参考链路，不进入产品级 fallback。
...
14. 董事长反馈明确反对为了低成本把硬件配置做得过于简单；因此本文后续所有成本压缩都必须同时说明是否损伤“功能做得再好一些”的产品战略意图。
...
6. 量产主线与研发对比基线可以并存，但研发对比基线不能偷偷进入产品 fallback。
7. 对会显著影响 `VLN / VLM` 端侧落地的大算力芯片路线，现阶段不能因为工程保守而过早排除出主线。
8. 如果纯视觉主线不过线，优先调整产品节奏，而不是回退到主动传感主线。

## 3.1 成本分配硬约束

整机物料成本上限已经明确为 `5000 到 6000 元人民币`，因此一代硬件选型不能只讨论算力上限，还必须同时讨论成本分配。

本节当前仍按已冻结成本基线组织。若 `KBT-57` 后续确认打开 `10000` 元 BOM 战略分支，则应新增一张并列表，而不是直接把下表覆盖掉。
...
- `KBT-57` 的 `10000` 元 BOM 战略分支必须单独回答“增加的钱买来哪些用户可感知能力、哪些服务闭环和哪些可靠性余量”，不得只把预算上修当成放松工程纪律。

### 3.1.1 `C1` 子桶拆分与当前价格校准

基于当前与供应链沟通得到的工作口径，`C1` 不能再只按“芯片 + 存储”的模糊写法讨论，而应至少拆成 4 个子桶：
...
按此粗算，仅 `RAM + Flash` 的成本压力已经很高：

| 配置 | 仅 `RAM + Flash` 成本 | 占 `C1` 上限 `1700` 的比例 | 当前判断 |
| --- | --- | --- | --- |
| `8GB RAM + 64GB Flash` | 约 `1035 元` | 约 `61%` | 可作为更激进的成本压缩讨论线，但当前不再作为默认量产线 |
| `12GB RAM + 32GB Flash` | 约 `1346 元` | 约 `79%` | 当前默认量产线，较好平衡内存与存储压力 |
| `12GB RAM + 64GB Flash` | 约 `1449 元` | 约 `85%` | 只适合作为边界验证线，不宜默认写成一代量产配置 |
| `16GB RAM + 64GB Flash` | 约 `1863 元` | 约 `110%` | 仅 `RAM + Flash` 已超过 `C1` 上限，不应再假定能自然落入一期量产桶 |
| `24GB RAM + 128GB Flash` | 约 `2898 元` | 约 `170%` | 只能作为前瞻验证线或未来 `Pro SKU` 候选 |

当前结论：

- `RAM` 已经成为 `C1` 最主要的成本驱动项，`Flash` 是次级压力，但在本地模型、日志、地图和 OTA 共存时仍不能忽略。
- 对一代量产，真正危险的不是“芯片单价略贵”，而是 `16GB+` 内存路线会直接击穿 `C1`。
- 因此，一代路线不应继续把“端侧 `4B / 7B / 8B` 多模态模型进入主线评估”和“默认量产配置”写成同一个概念。
- 当前建议把 `12GB RAM + 32GB Flash` 定义为量产默认线，把 `12GB + 64GB` 定义为边界验证线，把 `16GB + 64GB` 及以上定义为前瞻验证线或 `Pro SKU` 候选。

### 3.2 `Step22` 后的滚动修正

基于你在 `Step21` 与 `Step22` 中给出的方向性判断，当前先做 4 条滚动修正：

1. `C1 端侧算力与存储` 当前按“较上一轮提案上修约 `300` 元，且必须拆成 `SoC / RAM / Flash / 高速外围` 四个子桶”继续监测。
...
| `S2 相机与深度路线` | 双目 + 单目 `RGB` 组合、自研深度估计；深度相机；激光雷达 | 量产主线优先双目 + 单目组合 `3 到 5` 个相机，自研深度估计；深度相机和激光雷达仅作为研发对比基线与真值参考链路 | 符合你对简单相机配置、成本和体积的要求，也避免深度相机 / 雷达长期占据量产 BOM | 纯 `RGB` 路线对算法、标定和低光场景鲁棒性要求更高，且不过线时只能调整节奏 |
...
| `S5 储物仓、屏幕与整机结构路线` | 方案二：紧凑轻量头部 + 躯干内容屏；方案一：高集成度带屏头 | 当前默认主线冻结为方案二：头部承载主视觉、单主麦阵与小尺寸表情屏，`8 到 10` 寸内容显示屏下沉到躯干；方案一只在方案二的头颈高速链路、`EMI`、寿命与可维护性阶段门不过线时回退 | 优先保住三自由度头部的轻量化、灵动感和高端产品感，同时不把双麦阵修补式复杂度冻结进量产主线 | 头颈高速链路、线束寿命、`EMI`、外观比例与躯干内部空间会互相牵制 |
...
| `RK3576 + AI 协处理器` | 成本备线 | 更容易压低成本和功耗，适合作为 `5000 到 6000` 元量产压力下的备选路径 | 复杂场景下的算力与余量更紧，对 `VLN / VLM` 不够从容 | 作为成本备线保留 |
...
6. 端侧更重模型继续以 `4B / 7B / 8B + FP8` 的方式进入真实板级评估，但不再默认等价为一代量产标准配置；必须先通过 `C1` 成本门和 `C5` 功耗门。
7. 按当前 `RAM / Flash` 工作单价，建议把 `12GB RAM + 32GB Flash` 定义为量产默认线，把 `12GB + 64GB` 定义为边界验证线，把 `16GB + 64GB` 及以上定义为前瞻验证线或未来 `Pro SKU` 候选。
8. 任何 `16GB+` 内存路线若没有整机 `BOM` 上修、售价带上修或明确 `Pro SKU` 方案，不得进入一期量产定型。
9. 只要高算力主线持续把 `C1 端侧算力与存储` 压到 `1700 元` 以上，就必须同步给出其他成本桶的回收方案；若无法回收，则该路线不得直接成为一代量产定型。
10. 任何试图压低 `C4` 的方案，都必须同步展示其对 `C1 / C2 / C5` 的新增成本与新增风险。

当前推断：

- 一代当前最合理的推进方式，不是继续维持“大而散”的候选池，而是先用 `TTFT / TPS` 和成本桶把主线收窄，防止“只看模型上限，不看整机 `BOM` 与真实响应”。
- 当前 `C1` 的主矛盾已经从“选哪颗芯片”进一步转成“`RAM / Flash` 能不能在当前 `BOM` 框内支撑对应的模型路线”。
- 如果 `地瓜 S100 Pro（128 TOPS）` 能在 `C1` 成本桶、整机功耗和热设计内跑通，并在真实 `VLN / VLM` 任务上满足 `TTFT / TPS` 目标，它更有机会支撑多执行范式下离散决策、事件触发与连续调节的长期生命力。
...
3. 深度相机和激光雷达只作为研发阶段的对比基线与真值参考链路存在，不作为产品 fallback。
4. 若纯视觉路线在阶段门前不过线，优先调整产品节奏，而不是回退主动传感主线。
...
3. 不把语音体验问题推给云侧兜底。
4. `V1` 默认先按单主麦阵收敛，不把头 / 躯双麦阵同时冻结为量产默认。

### 6.2.1 头部 / 屏幕布局阶段门

当前建议：

1. `V1` 默认采用方案二“紧凑轻量头部 + 躯干内容屏”：头部承载主视觉、单主麦阵和小尺寸表情屏，内容显示屏下沉到躯干。
2. 方案二优先于方案一，核心原因不是器件更少，而是要优先保住三自由度头部的轻量化、角速度上限、灵动感和高端产品感。
3. 方案二必须通过头颈高速链路阶段门，至少覆盖视频链路稳定性、`EMI`、反复摆动寿命与装配 / 维护难度。
4. 只有当方案二在上述阶段门不过线，且必须引入过贵或过脆弱的线束 / 机构方案时，才回退到方案一的高集成度带屏头。
...
3. 音频：自研麦阵主线
4. 底盘：自研轮式底盘主线
5. 储物仓与结构：能力保留、机构重构，以及方案二“紧凑轻量头部 + 躯干内容屏”主线
6. 模型：端侧 `4B / 7B / 8B` 多模态模型与 `FP8` 量化继续进入主线评估，但一代量产默认配置先按 `12GB RAM + 32GB Flash` 约束，`12GB + 64GB` 及更高组合作为边界或前瞻验证
...
4. 深度相机与激光雷达作为研发对比基线
5. 智能家居与穿戴按白名单扩展备线
6. `12GB RAM + 64GB Flash` 作为边界验证线

### 7.3 观察线

1. 更复杂的深度传感器路线
2. `16GB RAM + 64GB Flash` 及以上的重本地多模态路线
...
为避免选型文档只停留在纸面，当前建议在 `G2` 前至少完成 8 项验证：

1. `地瓜 S100 Pro（128 TOPS 版本）` 与 `RK` 备线的功耗、热设计、内存带宽、模型迁移以及 `TTFT / TPS` 对比
2. 端侧 `4B / 7B / 8B` 模型在真实板级上的 `TTFT`、实验室峰值 `TPS`、热稳态持续 `TPS`、占用和 `FP8` 可行性验证，并同步记录其对应的 `RAM / Flash` 配置与 `C1` 占用
...
8. 完成 `8GB / 12GB / 16GB+` 三档内存路线与 `64GB / 128GB` 存储组合的 `C1 / C5 / 整机 BOM` 对比，并判断哪些只适合作为前瞻验证线或 `Pro SKU`
