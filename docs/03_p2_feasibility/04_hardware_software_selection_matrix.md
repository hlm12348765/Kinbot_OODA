...
| --- | --- | --- | --- |
| `8GB RAM + 64GB Flash` | 约 `1035 元` | 约 `61%` | 仍可作为量产默认线讨论，但已明显挤压 `SoC` 与板级外围空间 |
...
- 当前建议把 `8GB RAM + 64GB Flash` 定义为量产默认线，把 `12GB + 64GB` 定义为边界验证线，把 `16GB + 64GB` 及以上定义为前瞻验证线或 `Pro SKU` 候选。
...
7. 按当前 `RAM / Flash` 工作单价，建议把 `8GB RAM + 64GB Flash` 定义为量产默认线，把 `12GB + 64GB` 定义为边界验证线，把 `16GB + 64GB` 及以上定义为前瞻验证线或未来 `Pro SKU` 候选。
...
4. 底盘：自研轮式底盘主线
5. 储物仓：能力保留、机构重构主线
6. 模型：端侧 `4B / 7B / 8B` 多模态模型与 `FP8` 量化继续进入主线评估，但一代量产默认配置先按 `8GB RAM + 64GB Flash` 约束，`12GB+` 只作边界或前瞻验证
