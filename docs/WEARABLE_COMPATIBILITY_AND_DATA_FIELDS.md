...
1. 与 [docs/HEALTH_EVENT_PIPELINE.md](docs/HEALTH_EVENT_PIPELINE.md) 的关系：本文冻结信号源优先级、字段和新鲜度，作为健康事件管线的稳定输入。
2. 与 [docs/WORLD_STATE_SCHEMA.md](docs/WORLD_STATE_SCHEMA.md) 的关系：`device_binding_refs`、`vital_signal_sources`、`wearable_freshness_state` 等字段承接本文定义。
3. 与 [docs/SAFETY_COMPLIANCE_AUTHORIZATION_API.md](docs/SAFETY_COMPLIANCE_AUTHORIZATION_API.md) 的关系：数据新鲜度不足时，审批层必须支持补采、复核和降级。
4. 与 [docs/HARDWARE_SOFTWARE_SELECTION_MATRIX.md](docs/HARDWARE_SOFTWARE_SELECTION_MATRIX.md) 的关系：穿戴与家庭设备生态只通过接口约束本体，不反向锁死本体主控路线。
5. 与 `KBT-14` 的关系：`UWB` 当前只保留观察线地位，不阻塞 `KBT-15` 的主线冻结。

## 12. 当前提案中的待审点

以下 4 点目前作为 `KBT-15` 的正式评审入口：
...
4. 是否接受把 `UWB` 继续保留为观察线，不阻塞当前穿戴主线冻结。
